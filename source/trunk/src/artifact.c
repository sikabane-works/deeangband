...
bool create_artifact(creature_type *owner_ptr, object_type *object_ptr, bool a_scroll)
{
	...
	if (a_scroll)
	{
		char dummy_name[80] = "";
#ifdef JP
		cptr ask_msg = "‚±‚ÌƒA[ƒeƒBƒtƒ@ƒNƒg‚ð‰½‚Æ–¼•t‚¯‚Ü‚·‚©H";
#else
	...
	{
#ifdef JP
		if (artifact_bias) msg_format("‰^‚Ì•Î‚Á‚½ƒA[ƒeƒBƒtƒ@ƒNƒg: %dB", artifact_bias);
		else msg_print("ƒA[ƒeƒBƒtƒ@ƒNƒg‚É‰^‚Ì•Î‚è‚È‚µB");
#else
...
bool activate_random_artifact(creature_type *creature_ptr, object_type * object_ptr)
{
	...
	switch (object_ptr->xtra2)
	{
		...
		{
#ifdef JP
			msg_print("—lX‚ÈF‚Ì‰Î‰Ô‚ð”­‚µ‚Ä‚¢‚é...");
#else
		...
		case ACT_BA_MISS_3:
		{
			if (!get_aim_dir(creature_ptr, &dir)) return FALSE;
#ifdef JP
			msg_print("‚ ‚È‚½‚ÍƒGƒŒƒƒ“ƒg‚ÌƒuƒŒƒX‚ð“f‚¢‚½B");
#else
		...
		case ACT_BANISH_EVIL:
		{
			...
			{
#ifdef JP
				msg_print("ƒA[ƒeƒBƒtƒ@ƒNƒg‚Ì—Í‚ªŽ×ˆ«‚ð‘Å‚¿•¥‚Á‚½I");
#else
...
#ifdef JP
					msg_print("‚»‚ê‚ðƒRƒ“ƒgƒ[ƒ‹‚Å‚«‚È‚©‚Á‚½I");
#else
...
#ifdef JP
					msg_print("uNON SERVIAM! Wretch! ‚¨‘O‚Ì°‚ð’¸‚­‚¼Iv");
#else
		...
		case ACT_SUMMON_UNDEAD:
		{
			...
			{
#ifdef JP
				msg_print("—â‚½‚¢•—‚ª‚ ‚È‚½‚ÌŽü‚è‚É‚«Žn‚ß‚½B‚»‚ê‚Í•…”sL‚ð‰^‚ñ‚Å‚¢‚é...");
#else
				...
				if (pet)
#ifdef JP
					msg_print("ŒÃ‚¦‚ÌŽ€‚¹‚éŽÒ‹¤‚ª‚ ‚È‚½‚ÉŽd‚¦‚é‚½‚ß“y‚©‚çáS‚Á‚½I");
#else
...
#ifdef JP
					msg_print("Ž€ŽÒ‚ªáS‚Á‚½B–°‚è‚ð–W‚°‚é‚ ‚È‚½‚ð”±‚·‚é‚½‚ß‚ÉI");
#else
		...
		{
#ifdef JP
			msg_print("ŽŸŒ³‚Ì”à‚ªŠJ‚¢‚½B–Ú“I’n‚ð‘I‚ñ‚Å‰º‚³‚¢B");
#else
		...
		{
#ifdef JP
			msg_print("Žü‚è‚Ì‹óŠÔ‚ª˜c‚ñ‚Å‚¢‚é...");
#else
