...
#ifdef JP
		msg_format("%s%sの顔を見てしまった！", funny_desc[randint0(MAX_SAN_FUNNY)], m_name);
#else
...
#ifdef JP
	msg_format("%s%sの顔を見てしまった！", horror_desc[randint0(MAX_SAN_HORROR)], desc);
#else
...
#ifdef JP
		charges = get_quantity(format("一回分＄%d で何回分充填しますか？", price), MIN(creature_ptr->au / price, max_charges));
#else
