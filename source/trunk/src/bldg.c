...
void battle_creatures(void)
{
	...
	while (1)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			int num_taisei = 0;
			if(has_trait_raw(&r_ptr->flags, CF_RES_ACID)) num_taisei++;
			if(has_trait_raw(&r_ptr->flags, CF_RES_ELEC)) num_taisei++;
			if(has_trait_raw(&r_ptr->flags, CF_RES_FIRE)) num_taisei++;
			if(has_trait_raw(&r_ptr->flags, CF_RES_COLD)) num_taisei++;
			if(has_trait_raw(&r_ptr->flags, CF_RES_POIS)) num_taisei++;
			...
			else if (has_trait_raw(&r_ptr->flags, CF_INVULNER))
				power[i] = power[i] * 4 / 3;
			else if (has_trait_raw(&r_ptr->flags, CF_HEAL))
				power[i] = power[i] * 4 / 3;
			else if (has_trait_raw(&r_ptr->flags, CF_DRAIN_MANA))
				power[i] = power[i] * 11 / 10;
			if (has_trait_raw(&r_ptr->flags, CF_RAND_25))
				power[i] = power[i] * 9 / 10;
			if (has_trait_raw(&r_ptr->flags, CF_RAND_50))
				power[i] = power[i] * 9 / 10;
...
#endif
				creature_ptr->au += 1000000L * o_ptr->number;
				play_redraw |= (PR_GOLD);
				inven_item_increase(creature_ptr, i, -o_ptr->number);
...
#endif
				creature_ptr->au += 200000L * o_ptr->number;
				play_redraw |= (PR_GOLD);
				inven_item_increase(creature_ptr, i, -o_ptr->number);
				inven_item_describe(creature_ptr, i);
				inven_item_optimize(creature_ptr, i);
...
#endif
				creature_ptr->au += 100000L * o_ptr->number;
				play_redraw |= (PR_GOLD);
				inven_item_increase(creature_ptr, i, -o_ptr->number);
				inven_item_describe(creature_ptr, i);
				inven_item_optimize(creature_ptr, i);
...
#endif
				creature_ptr->au += (species_info[today_mon].level * 50 + 100) * o_ptr->number;
				play_redraw |= (PR_GOLD);
				inven_item_increase(creature_ptr, i, -o_ptr->number);
				inven_item_describe(creature_ptr, i);
				inven_item_optimize(creature_ptr, i);
...
#endif
				creature_ptr->au += (species_info[today_mon].level * 30 + 60) * o_ptr->number;
				play_redraw |= (PR_GOLD);
				inven_item_increase(creature_ptr, i, -o_ptr->number);
				inven_item_describe(creature_ptr, i);
				inven_item_optimize(creature_ptr, i);
...
#endif
				creature_ptr->au += (species_info[kubi_species_idx[j]].level+1) * 300 * o_ptr->number;
				play_redraw |= (PR_GOLD);
