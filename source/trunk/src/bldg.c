	...
	switch (cmd)
	{
		case BUILDING_FUNCTION_ARENA:
			if(arena_number == MAX_ARENA_MONS)
			{
				clear_bldg(5, 19);
#ifdef JP
				prt("ÉAÉäÅ[ÉiÇÃóDèüé“ÅI", 5, 0);
				prt("Ç®ÇﬂÇ≈Ç∆Ç§ÅIÇ†Ç»ÇΩÇÕëSÇƒÇÃìGÇì|ÇµÇ‹ÇµÇΩÅB", 7, 0); 
				prt("è‹ã‡Ç∆ÇµÇƒ $1,000,000 Ç™ó^Ç¶ÇÁÇÍÇ‹Ç∑ÅB", 8, 0);
				prt("", 10, 0);
				prt("", 11, 0);
				msg_print("ÉXÉyÅ[ÉXÉLÅ[Ç≈ë±çs");
#else
...
#ifdef JP
		if(kaeruka[i])
			c_put_str(col, "Ç©Ç¶ÇÈ", 14, 5+i*16);
		else
			c_put_str(col, "ÇÃÇ±Ç∑", 14, 5+i*16);
#else
		if(kaeruka[i])
...
#ifdef JP
	cptr suit[4] = {"Åö", "Åú", "Å˜", "Åı"};
	cptr card_grph[13][7] = {{"Ç`   %s     ",
				  "     ïœ     ",
				  "     ã     ",
				  "     îÿ     ",
				  "     ì{     ",
...
static int do_poker(void)
{
	...
	/* suteruno wo kimeru */
#ifdef JP
	prt("écÇ∑ÉJÅ[ÉhÇåàÇﬂÇƒâ∫Ç≥Ç¢(ï˚å¸Ç≈à⁄ìÆ, ÉXÉyÅ[ÉXÇ≈ëIë)ÅB", 0, 0);
#else
	prt("Stay witch? ", 0, 0);
#endif
		...
		if(get_string(tmp_str, out_val, 32))
		{
			...
			do
			{
...
#ifdef JP
					sprintf(tmp_str, "çïÉ_ÉCÉX: %d        çïÉ_ÉCÉX: %d", roll1, roll2);
					prt(tmp_str, 8, 3);
					sprintf(tmp_str, "ê‘É_ÉCÉX: %d", choice);
#else
				...
				case BUILDING_FUNCTION_CRAPS:  /* Game of Craps */
					...
					if((roll3 == 7) || (roll3 == 11))
						...
						do
						{
							...
							roll3 = roll1 +  roll2;
#ifdef JP
							sprintf(tmp_str, "èoñ⁄: %d %d          çáåv:      %d", roll1, roll2, roll3);
#else
				...
				case BUILDING_FUNCTION_SPIN_WHEEL:  /* Spin the Wheel Game */
					...
					strcpy(out_val, "");
#ifdef JP
					c_put_str(TERM_GREEN, "ÉãÅ[ÉåÉbÉg", 5, 2);
					get_string("âΩî‘ÅH (0-9)", out_val, 32);
#else
...
#ifdef JP
					c_put_str(TERM_GREEN, "É_ÉCÉXÅEÉXÉçÉbÉg", 5, 2);
					c_put_str(TERM_YELLOW, "ÉåÉÇÉì   ÉåÉÇÉì            2", 6, 37);
					c_put_str(TERM_YELLOW, "ÉåÉÇÉì   ÉåÉÇÉì   ÉåÉÇÉì   5", 7, 37);
					c_put_str(TERM_ORANGE, "ÉIÉåÉìÉW ÉIÉåÉìÉW ÉIÉåÉìÉW 10", 8, 37);
					c_put_str(TERM_UMBER, "åï       åï       åï       20", 9, 37);
					c_put_str(TERM_SLATE, "èÇ       èÇ       èÇ       50", 10, 37);
					c_put_str(TERM_VIOLET, "ÉvÉâÉÄ   ÉvÉâÉÄ   ÉvÉâÉÄ   200", 11, 37);
					c_put_str(TERM_RED, "É`ÉFÉäÅ[ É`ÉFÉäÅ[ É`ÉFÉäÅ[ 1000", 12, 37);
#else
...
#ifdef JP
		select_dungeon = choose_dungeon("Ç…ÉeÉåÉ|Å[Ég", 4, 0);
#else
...
#ifdef JP
		amt = (FLOOR_LEV)get_quantity(format("%sÇÃâΩäKÇ…ÉeÉåÉ|Å[ÉgÇµÇ‹Ç∑Ç©ÅH", dungeon_name + dungeon_info[select_dungeon].name), max_depth);
#else
...
#ifdef JP
		msg_print("ïΩçtâªÇÃãVéÆÇçsÇ»Ç¡ÇΩÅB");
#else
...
#ifdef JP
		if(has_trait(creature_ptr, TRAIT_ECHIZEN_TALK)) msg_print("ÅwÇ∆Ç…Ç©Ç≠ì¸Ç¡ÇƒÇ›ÇÊÇ§Ç∫Ç•ÅBÅx");
		if(has_trait(creature_ptr, TRAIT_CHARGEMAN_TALK)) msg_print("ÅwëSñ≈ÇµÇƒÇ‚ÇÈÇºÅIÅx");
#endif
