...
void update_lite(creature_type *cr_ptr)
{
	...
	if (p <= 0)
	{
		...
		cave_redraw_later(&current_floor_ptr->cave[cr_ptr->fy][cr_ptr->fx], cr_ptr->fy, cr_ptr->fx);
	}
#endif
...
void cave_alter_feat(int y, int x, int action)
{
	...
	if (!(feature_action_flags[action] & FAF_NO_DROP))
	{
		...
		if (found && change_floor_flag && creature_can_see_bold(player_ptr, y, x))
		{
#ifdef JP
			msg_print("何かを発見した！");
#else
	...
	if (feature_action_flags[action] & FAF_CRASH_GLASS)
	{
		...
		if (have_flag(old_f_ptr->flags, FF_GLASS) && change_floor_flag)
		{
...
void glow_deep_lava_and_bldg(void)
{
	...
	if (dungeon_info[current_floor_ptr->dun_type].flags1 & DF1_DARKNESS) return;
