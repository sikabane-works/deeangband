	...
	switch (object_ptr->tval)
	{
		...
		case TV_DIGGING:
		{
			...
			if (have_flag(flgs, TR_BRAND_POIS) || ((attacker_ptr->special_attack & (ATTACK_POIS)) && !thrown) || (mode == HISSATSU_POISON))
			{
				...
				/* Otherwise, take the damage */
				else if ((have_flag(flgs, TR_BRAND_POIS) || ((attacker_ptr->special_attack & (ATTACK_POIS)) && !thrown)) && (mode == HISSATSU_POISON))
				{
					if (mult < 35) mult = 35;
				}
				else
	...
	if (!(mpe_mode & MPE_STAYING))
	{
		...
		if (!(mpe_mode & MPE_DONT_SWAP_MON))
		{
			...
			if (om_idx > 0) /* Creature on old spot (or creature_ptr->riding) */
			{
				...
				om_ptr->fy = ny;
				om_ptr->fx = nx;
				update_creature_view(om_idx, TRUE);
			}
			...
			if (nm_idx > 0) /* Creature on new spot */
			{
				...
				nm_ptr->fy = oy;
				nm_ptr->fx = ox;
				update_creature_view(nm_idx, TRUE);
			}
		}
