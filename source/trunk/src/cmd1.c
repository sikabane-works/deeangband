...
bool player_can_enter(creature_type *cr_ptr, s16b feature, u16b mode)
{
	...
	if (cr_ptr->riding) return species_can_cross_terrain(feature, &species_info[creature_list[cr_ptr->riding].species_idx], mode | CEM_RIDING);
	...
	*/
	...
	if (c_ptr->m_idx && (m_ptr->ml || p_can_enter || p_can_kill_walls))
	{
		    ...
		    pattern_seq(cr_ptr, cr_ptr->fy, cr_ptr->fx, y, x) && (p_can_enter || p_can_kill_walls))
		{
			...
			else if (species_can_cross_terrain(cave[cr_ptr->fy][cr_ptr->fx].feat, r_ptr, 0))
			{
				do_past = TRUE;
			}
			else
