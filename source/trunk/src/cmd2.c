...
static void chest_trap(creature_type *creature_ptr, int y, int x, s16b object_idx)
{
	...
	if (trap & (CHEST_LOSE_STR))
	{
		...
		take_hit(NULL, creature_ptr, DAMAGE_NOESCAPE, damroll(1, 4), "毒針", NULL, -1);
#else
	...
	if ((trap & (CHEST_RUNES_OF_EVIL)) && o_ptr->k_idx)
	{
		...
		for (; nasty_tricks_count > 0; nasty_tricks_count--)
		{
			...
			{
#ifdef JP
				if (one_in_(6)) take_hit(NULL, creature_ptr, DAMAGE_NOESCAPE, damroll(5, 20), "破滅のトラップの宝箱", NULL, -1);
#else
	...
	if ((trap & (CHEST_EXPLODE)) && o_ptr->k_idx)
	{
		...
		sound(SOUND_EXPLODE);
#ifdef JP
		take_hit(NULL, creature_ptr, DAMAGE_ATTACK, damroll(5, 8), "爆発する箱", NULL, -1);
#else
