...
#ifdef JP
	if(record_stair) do_cmd_write_diary(DIARY_STAIR, 0-up_num, "階段を上った");
#else
...
void do_cmd_go_down(creature_type *creature_ptr)
{
	...
	else
	{
		...
		{
#ifdef JP
			if(fall_trap) do_cmd_write_diary(DIARY_STAIR, down_num, "落とし戸に落ちた");
			else do_cmd_write_diary(DIARY_STAIR, down_num, "階段を下りた");
#else
