...
#ifdef JP
		if (!get_spell(cr_ptr, &spell, "読む", o_ptr->sval, TRUE, use_realm))
#else
...
msg_print("An infernal sound echoed.");
	...
	else
	{
		...
		if (m_info[cr_ptr->cls_idx].spell_xtra & MAGIC_GAIN_EXP)
		{
			...
			{
				if ((current_floor_ptr->dun_level > 4) && ((current_floor_ptr->dun_level + 10) > cr_ptr->lev))
					exp_gain = 8;
			}
			else if (cur_exp < SPELL_EXP_EXPERT)
			{
				if (((current_floor_ptr->dun_level + 5) > cr_ptr->lev) && ((current_floor_ptr->dun_level + 5) > s_ptr->slevel))
					exp_gain = 2;
			}
			else if ((cur_exp < SPELL_EXP_MASTER) && !increment)
			{
				if (((current_floor_ptr->dun_level + 5) > cr_ptr->lev) && (current_floor_ptr->dun_level > s_ptr->slevel))
					exp_gain = 1;
			}
...
bool do_riding(creature_type *rider_ptr, bool force)
{
	...
	else
	{
		...
		if (!has_cf_creature(steed_ptr, CF_RIDING))
		{
#ifdef JP
	...
	/* Move the player */
	(void)move_creature_effect(rider_ptr, y, x, MPE_HANDLE_STUFF | MPE_ENERGY_USE | MPE_DONT_PICKUP | MPE_DONT_SWAP_MON);
