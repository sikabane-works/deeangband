...
static bool cast_wrath_of_the_god(creature_type *cr_ptr, int dam, int rad)
{
	...
	while (1)
	{
		...
		/* Stopped by monsters */
		if ((dir != 5) && cave[ny][nx].creature_idx != 0) break;
	...
	case 21:
		...
		if (cast)
		{
			...
			for (i = 0; i < 2; i++)
			{
				...
				if (!get_equipped_slot_ptr(cr_ptr, INVEN_SLOT_HAND, i+1)) break;
				o_ptr = get_equipped_slot_ptr(cr_ptr, INVEN_SLOT_HAND, i+1);
		...
		if (cast)
		{
			...
			for (i = 0; i < 3; i++)
			{
				...
				for (dir = 0; dir < 8; dir++)
				{
					...
					if (dir == 5) continue;
					if(cave[dy][dx].creature_idx) flag = TRUE;
				}
