...
static void cast_wonder(creature_type *creature_ptr, int dir)
{
	...
	else if(die < 86) cast_ball(creature_ptr, GF_ACID, dir, 40 + plev, 2);
	else if(die < 91) cast_ball(creature_ptr, GF_ICE, dir, 70 + plev, 3);
	else if(die < 96) cast_ball(creature_ptr, GF_FIRE, dir, 80 + plev, 3);
...
static void cast_invoke_spirits(creature_type *creature_ptr, int dir)
{
	...
	{
		cast_ball(creature_ptr, GF_POIS, dir, 20 + (plev / 2), 3);
	}
	else if(die < 51)
	{
	...
	{
		cast_ball(creature_ptr, GF_ELEC, dir, 30 + plev / 2, 2);
	}
	else if(die < 86)
	{
		cast_ball(creature_ptr, GF_ACID, dir, 40 + plev, 2);
	}
	else if(die < 91)
	{
		cast_ball(creature_ptr, GF_ICE, dir, 70 + plev, 3);
	}
	else if(die < 96)
	{
		cast_ball(creature_ptr, GF_FIRE, dir, 80 + plev, 3);
	}
	else if(die < 101)
	{
...
void wild_magic(creature_type *creature_ptr, int spell)
{
	...
	case 31:
		...
		break;
	case 32:
		cast_ball(creature_ptr, GF_CHAOS, 0, spell + 5, 1 + (spell / 10));
		break;
	case 33:
...
static void cast_shuffle(creature_type *creature_ptr)
{
	...
	else if(die < 96)
	{
		...
		if(get_aim_dir(creature_ptr, &dir)) charm_creature(creature_ptr, dir, MIN(creature_ptr->lev, 20));
	}
	else if(die < 101)
	{
#ifdef JP
...
static bool cast_wrath_of_the_god(creature_type *creature_ptr, int dam, int rad)
{
	...
	for (i = 0; i < b; i++)
	{
		...
		project(creature_ptr, 0, rad, y, x, dam, GF_DISINTEGRATE, PROJECT_JUMP | PROJECT_GRID | PROJECT_ITEM | PROJECT_KILL, -1);
	}
...
static cptr do_life_spell(creature_type *creature_ptr, int spell, int mode)
{
	...
	case 2:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball_hide(creature_ptr, GF_WOUNDS, dir, diceroll(dice, sides), 0);
			}
		}
		break;
	...
	case 9:
    ...
    
		{
			...
			{
				if(!get_aim_dir(creature_ptr, &dir)) return NULL;
				cast_ball_hide(creature_ptr, GF_WOUNDS, dir, diceroll(sides, dice), 0);
			}
		}
		break;
...
static cptr do_sorcery_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 28:
    ...
    
		{
			...
			if(cast)
			{
				project_hack(caster_ptr, GF_CHARM, power);
			}
		}
		break;
...
static cptr do_nature_spell(creature_type *caster_ptr, int spell, int mode)
{
...
#ifdef JP
					take_hit(NULL, caster_ptr, DAMAGE_NOESCAPE, diceroll(2, 2), "日の光", NULL, -1);
#else
	...
	case 26:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_COLD, dir, dam, rad);
			}
		}
		break;
	...
	case 27:
    ...
    
		{
			...
			{
				if(!get_aim_dir(caster_ptr, &dir)) return NULL;
				cast_ball(caster_ptr, GF_ELEC, dir, dam, rad);
				break;
			}
		}
		break;
	...
	case 28:
    ...
    
		{
			...
			{
				if(!get_aim_dir(caster_ptr, &dir)) return NULL;
				cast_ball(caster_ptr, GF_WATER, dir, dam, rad);
			}
		}
		break;
	...
	case 29:
    ...
    
		{
			...
			if(cast)
			{
				cast_ball(caster_ptr, GF_LITE, 0, dam, rad);
				wiz_lite(floor_ptr, caster_ptr, FALSE);
	...
	case 31:
    ...
    
		{
			...
			if(cast)
			{
				...
				project(caster_ptr, 0, b_rad, caster_ptr->fy, caster_ptr->fx, b_dam, GF_DISINTEGRATE, PROJECT_KILL | PROJECT_ITEM, -1);
			}
		}
		break;
	}
...
static cptr do_chaos_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 4:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_MISSILE, dir, diceroll(dice, sides) + base, rad);
	...
	case 6:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_DISINTEGRATE, dir,
	...
	case 10:
    ...
    
		{
			...
			if(cast)
			{
				...
				project(caster_ptr, 0, rad, caster_ptr->fy, caster_ptr->fx, dam, GF_SOUND, PROJECT_KILL | PROJECT_ITEM, -1);
			}
		}
		break;
	...
	case 12:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_FIRE, dir, dam, rad);
			}
		}
		break;
	...
	case 15:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_CHAOS, dir, dam, rad);
			}
		}
		break;
	...
	case 19:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_DISINTEGRATE, dir, dam, rad);
			}
		}
		break;
	...
	case 23:
    ...
    
		{
			if(cast)
			{
				...
				if(summon_specific((pet ? caster_ptr : NULL), caster_ptr->fy, caster_ptr->fx, (plev * 3) / 2, SUMMON_DEMON, mode))
				{
#ifdef JP
					msg_print("硫黄の悪臭が充満した。");
#else
	...
	case 26:
    ...
    
		{
			...
			if(cast)
			{
				cast_ball(caster_ptr, GF_FIRE, 0, dam, rad);
			}
		}
		break;
	...
	case 29:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_MANA, dir, dam, rad);
			}
		}
		break;
...
static cptr do_death_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 1:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_HELL_FIRE, dir, diceroll(dice, sides), rad);
				...
				if(one_in_(5))
				{
					...
					if(effect == 666)
						cast_ball_hide(caster_ptr, GF_DEATH_RAY, dir, plev * 200, 0);
					else if(effect < 500)
						cast_ball_hide(caster_ptr, GF_TURN_ALL, dir, plev, 0);
					else if(effect < 800)
						cast_ball_hide(caster_ptr, GF_OLD_CONF, dir, plev, 0);
					else
						cast_ball_hide(caster_ptr, GF_STUN, dir, plev, 0);
				}
			}
		}
		break;
	...
	case 3:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_POIS, dir, dam, rad);
			}
		}
		break;
	...
	case 8:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_OLD_DRAIN, dir, diceroll(dice, dice) + base, rad);
			}
		}
		break;
	...
	case 10:
    ...
    
		{
			...
			if(cast)
			{
				project(caster_ptr, 0, rad, caster_ptr->fy, caster_ptr->fx, dam, GF_POIS, PROJECT_KILL | PROJECT_ITEM, -1);
			}
		}
		break;
	...
	case 11:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball_hide(caster_ptr, GF_GENOCIDE, dir, power, 0);
			}
		}
		break;
	...
	case 16:
    ...
    
		{
			...
			if(cast)
			{
				...
				set_timed_trait(caster_ptr, TRAIT_AFRAID, 0);
			}
		}
		break;
	...
	case 23:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_DARK, dir, dam, rad);
			}
		}
		break;
	...
	case 30:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_HELL_FIRE, dir, dam, rad);
#ifdef JP
				take_hit(NULL, caster_ptr, DAMAGE_USELIFE, 20 + randint1(30), "地獄の劫火の呪文を唱えた疲労", NULL, -1);
#else
	...
	case 1:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_SPIDER, PC_ALLOW_GROUP))
				{
					if(fail)
					{
#ifdef JP
	...
	case 10:
    ...
    
		{
			...
			if(cast)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, (summon_lev * 3 / 2), SUMMON_PHANTOM, 0L))
				{
#ifdef JP
	...
	case 17:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_UNDEAD, 0L))
				{
					if(fail)
					{
#ifdef JP
	...
	case 18:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_HYDRA, 0L))
				{
					if(fail)
					{
#ifdef JP
	...
	case 20:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_HOUND, PC_ALLOW_GROUP))
				{
					if(fail)
					{
#ifdef JP
	...
	case 23:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_CYBER, 0L))
				{
					if(fail)
					{
#ifdef JP
	...
	case 27:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_DRAGON, 0L))
				{
					if(fail)
					{
#ifdef JP
	...
	case 29:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_DEMON, 0L))
				{
					if(fail)
					{
#ifdef JP
	...
	case 30:
    ...
    
		{
			if(cast || fail)
			{
				...
				if(trump_summoning(caster_ptr, 1, !fail, caster_ptr->fy, caster_ptr->fx, 0, SUMMON_HI_UNDEAD, PC_ALLOW_UNIQUE))
				{
					if(fail)
					{
#ifdef JP
...
static cptr do_arcane_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 25:
    ...
    
		{
			if(cast)
			{
				if(!summon_specific(NULL, caster_ptr->fy, caster_ptr->fx, plev, SUMMON_ELEMENTAL, (PC_ALLOW_GROUP | PC_FORCE_PET)))
				{
#ifdef JP
...
static cptr do_daemon_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 6:
    ...
    
		{
			if(cast)
			{
				if(!summon_specific(NULL, caster_ptr->fy, caster_ptr->fx, (plev * 3) / 2, SUMMON_MANES, (PC_ALLOW_GROUP | PC_FORCE_PET)))
				{
#ifdef JP
	...
	case 7:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_HELL_FIRE, dir, diceroll(dice, sides) + base, rad);
			}
		}
		break;
	...
	case 14:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_NETHER, dir, dam, rad);
			}
		}
		break;
	...
	case 15:
    ...
    
		{
			if(cast)
			{
				...
				if(summon_specific((pet ? caster_ptr : NULL), caster_ptr->fy, caster_ptr->fx, plev*2/3+randint1(plev/2), SUMMON_DEMON, mode))
				{
#ifdef JP
					msg_print("硫黄の悪臭が充満した。");
#else
	...
	case 18:
    ...
    
		{
			...
			{
				cast_ball(caster_ptr, GF_FIRE, 0, dam, rad);
				cast_ball_hide(caster_ptr, GF_LAVA_FLOW, 0, 2 + randint1(2), rad);
			}
		}
		break;
	...
	case 19:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_PLASMA, dir, dam, rad);
			}
		}
		break;
	...
	case 22:
    ...
    
		{
			...
			{
				if(!get_aim_dir(caster_ptr, &dir)) return NULL;
				cast_ball(caster_ptr, GF_NEXUS, dir, dam, rad);
			}
		}
		break;
	...
	case 23:
    ...
    
		{
			if(cast)
			{
				...
				cast_ball_hide(caster_ptr, GF_HAND_DOOM, dir, plev * 2, 0);
			}
		}
		break;
	...
	case 26:
    ...
    
		{
			...
			if(cast)
			{
				cast_ball(caster_ptr, GF_CHAOS, 0, dam, rad);
				cast_ball(caster_ptr, GF_CONFUSION, 0, dam, rad);
				cast_ball(caster_ptr, GF_CHARM, 0, power, rad);
			}
		}
		break;
	...
	case 30:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball_hide(caster_ptr, GF_BLOOD_CURSE, dir, dam, rad);
#ifdef JP
				take_hit(NULL, caster_ptr, DAMAGE_USELIFE, 20 + randint1(30), "血の呪い", NULL, -1);
#else
...
static cptr do_crusade_spell(creature_type *creature_ptr, int spell, int mode)
{
	...
	case 8:
    ...
    
		{
			...
			{
				if(!get_aim_dir(creature_ptr, &dir)) return NULL;
				cast_ball(creature_ptr, GF_AWAY_EVIL, dir, power, 0);
			}
		}
		break;
	...
	case 9:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(creature_ptr, GF_HOLY_FIRE, dir, diceroll(dice, sides) + base, rad);
			}
		}
		break;
	...
	case 14:
    ...
    
		{
			...
			{
				if(!get_aim_dir(creature_ptr, &dir)) return NULL;
				cast_bolt(creature_ptr, GF_ELEC, dam, 0, FALSE);
			}
		}
		break;
	...
	case 22:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(creature_ptr, GF_LITE, dir, dam, rad);
			}
		}
		break;
	...
	case 30:
    ...
    
		{
			...
			if(cast)
			{
				project(creature_ptr, 0, 1, creature_ptr->fy, creature_ptr->fx, b_dam, GF_HOLY_FIRE, PROJECT_KILL, -1);
...
static cptr do_music_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 2:
...

		{
			...
			if(cast)
			{
				...
				cast_bolt(caster_ptr,GF_SOUND, diceroll(dice, sides), 0, FALSE);
			}
		}
		break;
	...
	case 10:
...

		{
			...
			if(cont || cast)
			{
				project(caster_ptr, 0, rad, caster_ptr->fy, caster_ptr->fx, 0, GF_IDENTIFY, PROJECT_ITEM, -1);
			}
		}
	...
	case 19:
    ...
    
		{
			...
			if(cast)
			{
				...
				project(caster_ptr, 0, rad, caster_ptr->fy, caster_ptr->fx, power, GF_AWAY_ALL, PROJECT_KILL, -1);
			}
		}
		break;
	...
	case 30:
    ...
    
		{
			...
			if(cast)
			{
				...
				cast_ball(caster_ptr, GF_SOUND, dir, diceroll(dice, sides), rad);
			}
		}
		break;
...
static cptr do_hissatsu_spell(creature_type *caster_ptr, int spell, int mode)
{
	...
	case 0:
		...
		if(cast)
		{
			...
			project_hook(caster_ptr, MAX_RANGE_SUB,GF_ATTACK, dir, HISSATSU_2, PROJECT_STOP | PROJECT_KILL);
		}
		break;
	...
	case 29:
		...
		if(cast)
		{
			...
			project(caster_ptr, 0, (cave_have_flag_bold(floor_ptr, y, x, FF_PROJECT) ? 5 : 0), y, x, total_damage * 3 / 2, GF_METEOR, PROJECT_KILL | PROJECT_JUMP | PROJECT_ITEM, -1);
		}
		break;
...
#ifdef JP
			take_hit(NULL, caster_ptr, DAMAGE_NOESCAPE, 100 + randint1(100), "慶雲鬼忍剣を使った衝撃", NULL, -1);
#else
	...
	case 7:
		...
		if(cont)
		{
			...
			if((creature_ptr->class_skills.old_skills.magic_num2[2] <= 0) || (power >= 200))
			{
				...
				if(power)
				{
					project(creature_ptr, 0, rad, creature_ptr->fy, creature_ptr->fx, power, GF_HELL_FIRE,
		...
		if(cont)
		{
			...
			if(creature_ptr->class_skills.old_skills.magic_num2[2] <= 0)
			{
				...
				if(power)
				{
					...
					cast_ball(creature_ptr, GF_HELL_FIRE, dir, power, 1);
