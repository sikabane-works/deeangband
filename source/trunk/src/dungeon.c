...
static void check_music(creature_type *creature_ptr)
{
	...
	if(creature_ptr->spell_exp[REALM_MUSIC] < SPELL_EXP_BEGINNER)
		creature_ptr->spell_exp[REALM_MUSIC] += 5;
	else if(creature_ptr->spell_exp[REALM_MUSIC] < SPELL_EXP_SKILLED)
	{ if(one_in_(2) && (floor_ptr->floor_level > 4) && ((floor_ptr->floor_level + 10) > creature_ptr->lev)) creature_ptr->spell_exp[REALM_MUSIC] += 1; }
	else if(creature_ptr->spell_exp[REALM_MUSIC] < SPELL_EXP_EXPERT)
	{ if(one_in_(5) && ((floor_ptr->floor_level + 5) > creature_ptr->lev) && ((floor_ptr->floor_level + 5) > s_ptr->slevel)) creature_ptr->spell_exp[REALM_MUSIC] += 1; }
	else if(creature_ptr->spell_exp[REALM_MUSIC] < SPELL_EXP_MASTER)
	{ if(one_in_(5) && ((floor_ptr->floor_level + 5) > creature_ptr->lev) && (floor_ptr->floor_level > s_ptr->slevel)) creature_ptr->spell_exp[REALM_MUSIC] += 1; }
...
#ifdef JP
		sprintf(buf, "%sに降り立った。", map_name(GET_FLOOR_PTR(player_ptr)));
#else
