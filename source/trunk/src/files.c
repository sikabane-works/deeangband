	...
	else if (mode == 1)
	{
		...
		*statmsg = '\0';
		...
		if (gameover)
		{
			...
			{
#ifdef JP
				sprintf(statmsg, "…あなたは、%sの%d階で%sに殺された。", map_name(), current_floor_ptr->dun_level, gameover_from);
#else
		...
		else if (character_dungeon)
		{
			...
			{
#ifdef JP
				sprintf(statmsg, "…あなたは現在、 %s の %d 階で探索している。", map_name(), current_floor_ptr->dun_level);
#else
	...
	if (!done)
	{
		...
		if (!streq(gameover_from, "ripe") && !streq(gameover_from, "Seppuku"))
		{
			...
			else
			{
				if (streq(gameover_from, "途中終了"))
				{
					sprintf(tmp, "地下 %d 階で死んだ", current_floor_ptr->dun_level);
				}
				else
				{
					sprintf(tmp, "に地下 %d 階で殺された", current_floor_ptr->dun_level);
				}
			}
			center_string(buf, tmp);
