...
static errr init_info_raw(int fd, header *head)
{
	...
	C_MAKE(head->object_ptr, head->info_size, char);

	/* Read the "*_info" array */
	fd_read(fd, head->object_ptr, head->info_size);
...
static errr init_info2(cptr filename, header *head, void **info, char **name, char **text, char **tmp, char **tag)
{
	...
	if(err)
	{
		...
		if(fd >= 0)
		{
			...
			fd_write(fd, head->object_ptr, head->info_size);
		...
		//TODO C_KILL(head->object_ptr, head->info_size, char);
...
static errr init_info(cptr filename, header *head, void **info, char **name, char **text, char **tag)
{
	...
	if(err)
	{
		...
		/* Free the "*_info" array */
		C_KILL(head->object_ptr, head->info_size, char);
...
static errr init_other(void)
{
	...
	/* Message variables */
	C_MAKE(message__ptr, MESSAGE_MAX, int);
...
void init_angband(void)
{
	...
	path_build(buf, sizeof(buf), ANGBAND_DIR_FILE, TEXT_FILES_NEWS);
	...
	if(load_lua(&KEYWORDS, ANGBAND_DIR_FILE, "keywords.lua"))
	{
