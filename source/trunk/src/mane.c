	...
	*sn = (-1);
...
#ifdef JP
	(void)strnfmt(out_val, 78, "(%c-%c, '*'Ç≈àÍóó, ESC) Ç«ÇÃ%sÇÇ‹ÇÀÇ‹Ç∑Ç©ÅH", I2A(0), I2A(num - 1), p);
#else
	...
	while (!flag)
	{
...
#ifdef JP
				put_str(format("%4s å¯â ", KW_FAIL), y, x + 36);
#else
		...
		if(ask)
		{
			char tmp_val[160];
#ifdef JP
			(void)strnfmt(tmp_val, 78, "%sÇÇ‹ÇÀÇ‹Ç∑Ç©ÅH", trait_info[creature_ptr->mane_spell[i]].title);
#else
