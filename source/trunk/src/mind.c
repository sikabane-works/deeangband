...
if(!b) msg_print("安全な気がする。");
	...
	case 1:
		...
		if(randint1(100) < plev * 2)
			cast_beam_(creature_ptr, GF_PSI, dir, diceroll(3 + ((plev - 1) / 4), (3 + plev / 15)));
		else
...
msg_print("精神を捻じ曲げる波動を発生させた！");
	...
	case 12:
		...
		cast_beam_(creature_ptr, GF_PSY_SPEAR, dir, randint1(plev*3)+plev*3);
		break;
	case 13:
	{
...
static bool cast_force_spell(creature_type *creature_ptr, int spell)
{
	...
	case 7:
	{
		...
		cast_beam_(creature_ptr, GF_MISSILE, dir, dam);
	...
	case 9:
	{
		...
		if(!projectable(floor_ptr, project_length, creature_ptr->fy, creature_ptr->fx, target_row, target_col)) break;
...
msg_print("御用でございますが、御主人様？");
	...
	case 12:
		...
		cast_beam_(creature_ptr, GF_MANA, dir, diceroll(10 + (plev / 2) + boost * 3 / 10, 15));
		break;
	case 13:
...
msg_format("There are too many mirrors to control!");
	...
	case 7:
	  if(!get_aim_dir(creature_ptr, &dir)) return FALSE;
	  (void)cast_beam_(creature_ptr, GF_AWAY_ALL, dir , plev);
	  break;
	/* mirror clashing */
	case 8:
	  if(!get_aim_dir(creature_ptr, &dir)) return FALSE;
	...
	case 10:
	  if(!get_aim_dir(creature_ptr, &dir)) return FALSE;
	  cast_beam_(creature_ptr, GF_SEEKER,dir, diceroll(11+(plev-5)/4,8));
	  break;
	/* seal of mirror */
	case 11:
	...
	case 13:
	  if(!get_aim_dir(creature_ptr, &dir)) return FALSE;
	  cast_beam_(creature_ptr, GF_SUPER_RAY,dir, 150+randint1(2*plev));
	  break;
	/* illusion light */
	case 14:
