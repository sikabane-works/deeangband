...
errr get_species_num_prep2(creature_type *summoner_ptr, creature_hook_type2 creature_hook, creature_hook_type2 creature_hook2)
{
	...
	for (i = 0; i < alloc_race_size; i++)
	{
		...
		if (current_floor_ptr->dun_level && (!inside_quest || is_fixed_quest_idx(inside_quest)) && !restrict_monster_to_dungeon(entry->index) && !gamble_arena_mode)
		{
			int hoge = entry->prob2 * dungeon_info[current_floor_ptr->dun_type].special_div;
			entry->prob2 = hoge / 64;
			if (randint0(64) < (hoge & 0x3f)) entry->prob2++;
...
void sanity_blast(creature_type *watcher_ptr, creature_type *m_ptr, bool necro)
{
	...
	if (gamble_arena_mode || !character_dungeon) return;
...
void update_mon(int m_idx, bool full)
{
	...
	if (flag)
	{
		...
		if (!m_ptr->ml)
		{
			...
			if (disturb_near && (projectable(player_ptr->fy, player_ptr->fx, m_ptr->fy, m_ptr->fx)))
			{
...
msg_print("守りのルーンが壊れた！");
	...
	creature_ptr->floor_id = current_floor_ptr->floor_id;
	creature_ptr->fy = y;
	creature_ptr->fx = x;
