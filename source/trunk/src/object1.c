...
cptr mention_use_idx(creature_type *creature_ptr, SLOT_ID slot, int num)
{
	...
	{
#ifdef JP
		case SLOT_ID_INVENTORY: p = "èäéù"; break;
		case SLOT_ID_ARMS: p = "òrïî"; break;
		case SLOT_ID_FEET: p = "ãrïî"; break;
		case SLOT_ID_TAIL: p = "îˆïî"; break;
		case SLOT_ID_INTAKE: p = "ëÃì‡"; break;
		case SLOT_ID_AMMO: p = "ñÓíe"; break;
		case SLOT_ID_AMULET: p = "åÏïÑ"; break;
		case SLOT_ID_LITE: p = "åıåπ"; break;
		case SLOT_ID_INSTRUMENT: p = "äyäÌ"; break;
		case SLOT_ID_BODY: p = "ëÃ"; break;
		case SLOT_ID_OUTER: p = "è„àﬂ"; break;
#else
...
cptr describe_use(creature_type *creature_ptr, int i)
{
	...
	{
#ifdef JP
		case SLOT_ID_HAND:  p = creature_ptr->heavy_wield[0] ? "â^î¿íÜÇÃ" : ((creature_ptr->two_handed && creature_ptr->can_melee[0]) ? "óºéËÇ…ëïîıÇµÇƒÇ¢ÇÈ" : (has_trait(creature_ptr, TRAIT_LEFT_HANDER) ? "ç∂éËÇ…ëïîıÇµÇƒÇ¢ÇÈ" : "âEéËÇ…ëïîıÇµÇƒÇ¢ÇÈ")); break;
		case SLOT_ID_BOW:   p = (adj_str_equipping_weight[creature_ptr->stat_ind[STAT_STR]] < creature_ptr->inventory[i].weight / 10) ? "éùÇ¬ÇæÇØÇ≈ê∏àÍîtÇÃ" : "éÀåÇópÇ…ëïîıÇµÇƒÇ¢ÇÈ"; break;
		case SLOT_ID_RING: p = (has_trait(creature_ptr, TRAIT_LEFT_HANDER) ? "ç∂éËÇÃéwÇ…ÇÕÇﬂÇƒÇ¢ÇÈ" : "âEéËÇÃéwÇ…ÇÕÇﬂÇƒÇ¢ÇÈ"); break;
		case SLOT_ID_AMULET:	p = "éÒÇ…Ç©ÇØÇƒÇ¢ÇÈ";		break;
		case SLOT_ID_LITE:	p = "åıåπÇ…ÇµÇƒÇ¢ÇÈ";		break;
		case SLOT_ID_BODY:	p = "ëÃÇ…íÖÇƒÇ¢ÇÈ";			break;
		case SLOT_ID_OUTER:	p = "êgÇ…Ç‹Ç∆Ç¡ÇƒÇ¢ÇÈ";		break;
		case SLOT_ID_HEAD:	p = "ì™Ç…Ç©Ç‘Ç¡ÇƒÇ¢ÇÈ";		break;
		case SLOT_ID_ARMS:	p = "éËÇ…Ç¬ÇØÇƒÇ¢ÇÈ";		break;
		case SLOT_ID_FEET:	p = "ë´Ç…ÇÕÇ¢ÇƒÇ¢ÇÈ";		break;
		case SLOT_ID_INTAKE:	p = "ëÃì‡Ç…éÊÇËçûÇÒÇ≈Ç¢ÇÈ";	break;
