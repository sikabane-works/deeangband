	...
	switch (slot)
	{
		...
		case INVEN_SLOT_HAND:  
			...
			else
			{
				...
				{
					case 1:
						p = has_trait(cr_ptr, TRAIT_HUMANOID) ? "右手" : "第１手";
						break;
					case 2:
						p = has_trait(cr_ptr, TRAIT_HUMANOID) ? "左手" : "第２手";
						break;
					case 3:
						p = "第３手";
						break;
		...
		case INVEN_SLOT_BOW:
			p = "射撃用";
//			p = (adj_str_equipping_weight[cr_ptr->stat_ind[STAT_STR]] < cr_ptr->inventory[i].weight / 10) ? "運搬中" : "射撃用"; break;
			break;
#else
		case INVEN_SLOT_BOW:
			p = "Shooting";
//			p = (adj_str_equipping_weight[cr_ptr->stat_ind[STAT_STR]] < cr_ptr->inventory[i].weight / 10) ? "Just holding" : "Shooting"; break;
		...
		case INVEN_SLOT_RING:
			...
			{
				case 1:
					p = has_trait(cr_ptr, TRAIT_HUMANOID) ? "右指" : "第１指";
					break;
				case 2:
					p = has_trait(cr_ptr, TRAIT_HUMANOID) ? "左指" : "第２指";
					break;
				default:
					p = "指？";
					break;
		...
		case INVEN_SLOT_HEAD:
			...
			{
				case 1:
					p = has_trait(cr_ptr, TRAIT_HUMANOID) ? "頭部" : "第１頭";
					break;
				case 2:
					p = "第２頭";
					break;
...
cptr describe_use(creature_type *cr_ptr, int i)
{
	...
	{
#ifdef JP
		case INVEN_SLOT_HAND:  p = cr_ptr->heavy_wield[0] ? "運搬中の" : ((cr_ptr->two_handed && cr_ptr->can_melee[0]) ? "両手に装備している" : (has_trait(cr_ptr, TRAIT_LEFT_HANDER) ? "左手に装備している" : "右手に装備している")); break;
#else
...
#ifdef JP
		case INVEN_SLOT_BOW:   p = (adj_str_equipping_weight[cr_ptr->stat_ind[STAT_STR]] < cr_ptr->inventory[i].weight / 10) ? "持つだけで精一杯の" : "射撃用に装備している"; break;
#else
...
#ifdef JP
		case INVEN_SLOT_RING: p = (has_trait(cr_ptr, TRAIT_LEFT_HANDER) ? "左手の指にはめている" : "右手の指にはめている"); break;
#else
