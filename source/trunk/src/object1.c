...
cptr mention_use_idx(creature_type *creature_ptr, INVENTORY_ID slot, int num)
{
	...
	{
#ifdef JP
		case INVENTORY_ID_INVENTORY: p = "èäéù"; break;
		case INVENTORY_ID_ARMS: p = "òrïî"; break;
		case INVENTORY_ID_FEET: p = "ãrïî"; break;
		case INVENTORY_ID_TAIL: p = "îˆïî"; break;
		case INVENTORY_ID_INTAKE: p = "ëÃì‡"; break;
		case INVENTORY_ID_AMMO: p = "ñÓíe"; break;
		case INVENTORY_ID_AMULET: p = "åÏïÑ"; break;
		case INVENTORY_ID_LITE: p = "åıåπ"; break;
		case INVENTORY_ID_INSTRUMENT: p = "äyäÌ"; break;
		case INVENTORY_ID_BODY: p = "ëÃ"; break;
		case INVENTORY_ID_OUTER: p = "è„àﬂ"; break;
#else
...
cptr describe_use(creature_type *creature_ptr, int i)
{
	...
	{
#ifdef JP
		case INVENTORY_ID_HAND:  p = creature_ptr->heavy_wield[0] ? "â^î¿íÜÇÃ" : ((creature_ptr->two_handed && creature_ptr->can_melee[0]) ? "óºéËÇ…ëïîıÇµÇƒÇ¢ÇÈ" : (has_trait(creature_ptr, TRAIT_LEFT_HANDER) ? "ç∂éËÇ…ëïîıÇµÇƒÇ¢ÇÈ" : "âEéËÇ…ëïîıÇµÇƒÇ¢ÇÈ")); break;
		case INVENTORY_ID_BOW:   p = (adj_str_equipping_weight[creature_ptr->stat_ind[STAT_STR]] < creature_ptr->inventory[i].weight / 10) ? "éùÇ¬ÇæÇØÇ≈ê∏àÍîtÇÃ" : "éÀåÇópÇ…ëïîıÇµÇƒÇ¢ÇÈ"; break;
		case INVENTORY_ID_RING: p = (has_trait(creature_ptr, TRAIT_LEFT_HANDER) ? "ç∂éËÇÃéwÇ…ÇÕÇﬂÇƒÇ¢ÇÈ" : "âEéËÇÃéwÇ…ÇÕÇﬂÇƒÇ¢ÇÈ"); break;
		case INVENTORY_ID_AMULET:	p = "éÒÇ…Ç©ÇØÇƒÇ¢ÇÈ";		break;
		case INVENTORY_ID_LITE:	p = "åıåπÇ…ÇµÇƒÇ¢ÇÈ";		break;
		case INVENTORY_ID_BODY:	p = "ëÃÇ…íÖÇƒÇ¢ÇÈ";			break;
		case INVENTORY_ID_OUTER:	p = "êgÇ…Ç‹Ç∆Ç¡ÇƒÇ¢ÇÈ";		break;
		case INVENTORY_ID_HEAD:	p = "ì™Ç…Ç©Ç‘Ç¡ÇƒÇ¢ÇÈ";		break;
		case INVENTORY_ID_ARMS:	p = "éËÇ…Ç¬ÇØÇƒÇ¢ÇÈ";		break;
		case INVENTORY_ID_FEET:	p = "ë´Ç…ÇÕÇ¢ÇƒÇ¢ÇÈ";		break;
		case INVENTORY_ID_INTAKE:	p = "ëÃì‡Ç…éÊÇËçûÇÒÇ≈Ç¢ÇÈ";	break;
...
int show_item_list(int target_item, creature_type *creature_ptr, FLAGS_32 flags, bool (*hook)(creature_type *creature_ptr, object_type *object_ptr))
{
	...
	if(!k)
	{
		...
		{
#if JP
			put_str("[âΩÇ‡ëïîıÇ≈Ç´Ç»Ç¢]", 1, flags & SHOW_ITEM_RIGHT_SET ? wid - 23 : 1);
#else
...
void py_pickup_aux(creature_type *creature_ptr, OBJECT_ID object_idx)
{
	int slot, i;
/*
 * ÉAÉCÉeÉÄÇèEÇ¡ÇΩç€Ç…ÅuÇQÇ¬ÇÃÉPÅ[ÉLÇéùÇ¡ÇƒÇ¢ÇÈÅv
 * "You have two cakes." Ç∆ÉAÉCÉeÉÄÇèEÇ¡ÇΩå„ÇÃçáåvÇÃÇ›ÇÃï\é¶Ç™ÉIÉäÉWÉiÉã
 * ÇæÇ™ÅAà·òaä¥Ç™
 * Ç†ÇÈÇ∆Ç¢Ç§éwìEÇÇ§ÇØÇΩÇÃÇ≈ÅAÅuÅ`ÇèEÇ¡ÇΩÅAÅ`ÇéùÇ¡ÇƒÇ¢ÇÈÅvÇ∆Ç¢Ç§ï\é¶
 * Ç…Ç©Ç¶ÇƒÇ†ÇÈÅBÇªÇÃÇΩÇﬂÇÃîzóÒÅB
 */
	char object_name[MAX_NLEN];
	...
	{
		msg_format("Ç±Ç§ÇµÇƒÅA%sÇÕÅwÉNÉäÉÄÉ]ÉìÅxÇéËÇ…ì¸ÇÍÇΩÅB", creature_ptr->name);
		msg_print("ÇµÇ©Çµç°ÅAÅwç¨ì◊ÇÃÉTÅ[ÉyÉìÉgÅxÇÃï˙Ç¡ÇΩÉNÉäÅ[É`ÉÉÅ[Ç™ÅA");
		msg_format("%sÇ…èPÇ¢Ç©Ç©ÇÈÅDÅDÅD", creature_ptr->name);
	}
	else
	{
		if(plain_pickup) msg_format("%s(%c)ÇéùÇ¡ÇƒÇ¢ÇÈÅB",object_name, index_to_label(slot));
		else
		{
			if(object_ptr->number > hirottakazu)
			    msg_format("%sèEÇ¡ÇƒÅA%s(%c)ÇéùÇ¡ÇƒÇ¢ÇÈÅB",kazu_str, object_name, index_to_label(slot));
			else
				msg_format("%s(%c)ÇèEÇ¡ÇΩÅB", object_name, index_to_label(slot));
		}
	}
