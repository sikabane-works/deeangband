...
static bool find_space(floor_type *floor_ptr, COODINATES *y, COODINATES *x, COODINATES height, COODINATES width)
{
	...
	/* Normal dungeon */
	if(!(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_NO_CAVE))
	{
		/* Choose a random one */
...
static bool build_type1(floor_type *floor_ptr)
{
	...
	bool light;
	cave_type *c_ptr;
	bool curtain = (dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_CURTAIN) &&
		one_in_((dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_NO_CAVE) ? 48 : 512);
	...
	/* Choose lite or dark */
	light = ((floor_ptr->depth <= randint1(25)) && !(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS));
	...
	{
		bool curtain2 = (dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_CURTAIN) &&
			one_in_((dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_NO_CAVE) ? 2 : 128);
	...
	switch (randint0(4))
	{
		...
		case 3:
		{
			...
			if(one_in_(3))
			{
				...
				if(one_in_(3))
				{
					int door_type = ((dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_CURTAIN) &&
						one_in_((dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_NO_CAVE) ? 16 : 256)) ? DOOR_CURTAIN :
						((dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_GLASS_DOOR) ? DOOR_GLASS_DOOR : DOOR_DOOR);
...
static bool vault_aux_jelly(SPECIES_ID species_idx)
{
	...
	if(has_trait_species(species_ptr, TRAIT_KILL_BODY) && !has_trait_species(species_ptr, TRAIT_NEVER_BLOW)) return FALSE;

	/* Also decline evil jellies (like death molds and shoggoths) */
	if(is_enemy_of_good_species(species_ptr)) return FALSE;
...
static bool vault_aux_chapel_g(SPECIES_ID species_idx)
{
	...
	if(species_ptr->d_char == 'A') return TRUE;
...
static bool vault_aux_symbol_e(SPECIES_ID species_idx)
{
	...
	if(is_enemy_of_evil_species(species_ptr)) return FALSE;
...
static bool vault_aux_giant(SPECIES_ID species_idx)
{
	...
	/* Require giant */
	if(!has_trait_species(species_ptr, TRAIT_GIANT)) return FALSE;
...
static bool vault_aux_dragon(SPECIES_ID species_idx)
{
	...
	/* Require dragon */
	if(!has_trait_species(species_ptr, TRAIT_DRAGON)) return FALSE;

	/* Hack -- Require correct "breath attack" */
	//TODO if(species_ptr->flags4 != vault_aux_dragon_mask4) return FALSE;
...
static bool build_type5(floor_type *floor_ptr)
{
	...
	int cur_nest_type = pick_vault_type(floor_ptr, nest_types, dungeon_info[floor_ptr->dungeon_id].nest);
	vault_aux_type *n_ptr;
	...
	for (y = y1; y <= y2; y++)
	{
		for (x = x1; x <= x2; x++)
		{
			...
			(void)place_creature_fixed_species(NULL, floor_ptr, y, x, species_idx, 0L);
...
static bool build_type6(floor_type *floor_ptr)
{
	...
	int cur_pit_type = pick_vault_type(floor_ptr, pit_types, dungeon_info[floor_ptr->dungeon_id].pit);
	vault_aux_type *n_ptr;
	...
	/* Center creature */
	place_creature_fixed_species(NULL, floor_ptr, yval, xval, what[7], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval, xval+1, what[6], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval, xval-1, what[6], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval-1, xval, what[6], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval+1, xval, what[6], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval-1, xval-1, what[5], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval+1, xval-1, what[5], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval-1, xval+1, what[5], PC_NO_KAGE);
	place_creature_fixed_species(NULL, floor_ptr, yval+1, xval+1, what[5], PC_NO_KAGE);
	...
	while(k <= y2 - yval|| k <= x2 - xval)
	{
		for(i = -k; i <= k; i++)
		{
			if(yval + i >= y1 && yval + i <= y2 && xval - k >= x1) place_creature_fixed_species(NULL, floor_ptr, yval + i, xval - k, what[k<7 ? 7-k : 0], PC_NO_KAGE);
			if(yval + i >= y1 && yval + i <= y2 && xval + k <= x2) place_creature_fixed_species(NULL, floor_ptr, yval + i, xval + k, what[k<7 ? 7-k : 0], PC_NO_KAGE);
		}
		for(i = -k + 1; i <= k - 1; i++)
		{
			if(yval - k >= y1 && xval + i >= x1 && xval + i <= x2) place_creature_fixed_species(NULL, floor_ptr, yval - k, xval + i, what[k<7 ? 7-k : 0], PC_NO_KAGE);
			if(yval + k <= y2 && xval + i >= x1 && xval + i <= x2) place_creature_fixed_species(NULL, floor_ptr, yval + k, xval + i, what[k<7 ? 7-k : 0], PC_NO_KAGE);
		}
		k++;
	}
...
static bool build_type9(floor_type *floor_ptr)
{
	...
	if((floor_ptr->depth <= randint1(25)) && !(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS)) light = TRUE;
...
void build_cavern(floor_type *floor_ptr)
{
	...
	light = done = FALSE;
	if((floor_ptr->depth <= randint1(50)) && !(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS)) light = TRUE;
	...
	* can make teleport traps instadeaths... */
	...
	for (x = 1; x < xsize; ++x)
	{
		for (y = 1; y < ysize; ++y)
		{
			...
			{
				if(!(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS)) floor_ptr->cave[y0 + y - yhsize][x0 + x - xhsize].info |= CAVE_GLOW;
			}
		}
	}
...
void build_maze_vault(floor_type *floor_ptr, COODINATES x0, COODINATES y0, COODINATES xsize, COODINATES ysize, bool is_vault)
{
	...
	/* Choose lite or dark */
	light = ((floor_ptr->depth <= randint1(25)) && is_vault && !(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS));
...
static bool build_type10(floor_type *floor_ptr)
{
	...
	while ((dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_NO_CAVE) &&
...
static bool build_type11(floor_type *floor_ptr)
{
	...
	/* Occasional light */
	if((randint1(floor_ptr->depth) <= 15) && !(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS)) light = TRUE;
...
static bool build_type12(floor_type *floor_ptr)
{
	...
	/* Occasional light */
	if((randint1(floor_ptr->depth) <= 5) && !(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_DARKNESS)) light = TRUE;
...
static bool build_type13(floor_type *floor_ptr)
{
	...
	/* Only in Angband */
	if(floor_ptr->dungeon_id != DUNGEON_ANGBAND) return FALSE;
	...
	for (i = 0; placing[i][2] >= 0; i++)
	{
		y = yval + placing[i][0];
		x = xval + placing[i][1];
		place_creature_fixed_species(NULL, floor_ptr, y, x, what[placing[i][2]], PC_NO_KAGE);
	}
	...
	case 1: /* 4 lite breathers + potion */
		{
			...
			for (dir1 = 4; dir1 < 8; dir1++)
			{
				...
				if(species_idx) place_creature_fixed_species(NULL, floor_ptr, y, x, species_idx, PC_ALLOW_SLEEP);
			...
			/* Place a potion */
			place_object(floor_ptr, yval, xval, AM_NO_FIXED_ART); // TODO: kind_is_potion
	...
	case 2: /* 1 lite breather + random object */
		{
			...
			if(species_idx) place_creature_fixed_species(NULL, floor_ptr, yval, xval, species_idx, 0L);
	...
	case 3: /* 4 shards breathers + 2 potions */
		{
			...
			for (dir1 = 4; dir1 < 8; dir1++)
			{
				...
				y = yval + ddy_ddd[dir1];
				x = xval + ddx_ddd[dir1];
				if(species_idx) place_creature_fixed_species(NULL, floor_ptr, y, x, species_idx, 0L);
			}

			/* Place two potions */
			if(one_in_(2))
			{
				place_object(floor_ptr, yval, xval - 1, AM_NO_FIXED_ART); // TODO kind_is_potion
				place_object(floor_ptr, yval, xval + 1, AM_NO_FIXED_ART); // TODO kind_is_potion
			}
			else
			{
				place_object(floor_ptr, yval - 1, xval, AM_NO_FIXED_ART); // TODO kind_is_potion
				place_object(floor_ptr, yval + 1, xval, AM_NO_FIXED_ART); // TODO kind_is_potion
			}
...
bool generate_rooms(floor_type *floor_ptr)
{
	...
	if(ironman_rooms && !((dungeon_info[floor_ptr->dungeon_id].flags1 & (DF1_BEGINNER | DF1_CHAMELEON | DF1_SMALLEST))))
	{
		for (i = 0; i < ROOM_T_MAX; i++)
		{
	...
	/* Forbidden vaults */
	else if(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_NO_VAULT)
	{
	...
	else if(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_CAVE)
	{
	...
	if(!(dungeon_info[floor_ptr->dungeon_id].flags1 & DF1_GLASS_ROOM))
	{
