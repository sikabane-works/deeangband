...
static void project_creature_aux(creature_type *caster_ptr, creature_type *target_ptr, EFFECT_ID typ, POWER dam, int spell)
{
	...
	case DO_EFFECT_PSI:
		...
		{
			if(seen_msg) msg_format("%sは%sが見えないので影響されない！", target_ptr, caster_ptr);
			skipped = TRUE;
			break;
		}
	...
	case DO_EFFECT_CAPTURE:
		{
			...
			if((floor_ptr->quest && (quest[floor_ptr->quest].type == QUEST_TYPE_KILL_ALL) && !is_pet(player_ptr, target_ptr)) ||
				(has_trait(target_ptr, TRAIT_UNIQUE)) || has_trait(target_ptr, TRAIT_NAZGUL) || has_trait(target_ptr, TRAIT_UNIQUE2) || (has_trait(target_ptr, TRAIT_QUESTOR)) || target_ptr->parent_creature_idx)
			{
	...
	case DO_EFFECT_RESTORE_EXP: restore_exp(caster_ptr); break;

	case DO_EFFECT_DEC_STR: do_dec_stat(caster_ptr, STAT_STR); break;
	case DO_EFFECT_DEC_INT: do_dec_stat(caster_ptr, STAT_INT); break;
	case DO_EFFECT_DEC_WIS: do_dec_stat(caster_ptr, STAT_WIS); break;
	case DO_EFFECT_DEC_DEX: do_dec_stat(caster_ptr, STAT_DEX); break;
	...
	case DO_EFFECT_INC_STR: do_inc_stat(caster_ptr, STAT_STR); break;
	case DO_EFFECT_INC_INT: do_inc_stat(caster_ptr, STAT_INT); break;
	case DO_EFFECT_INC_WIS: do_inc_stat(caster_ptr, STAT_WIS); break;
	case DO_EFFECT_INC_DEX: do_inc_stat(caster_ptr, STAT_DEX); break;
