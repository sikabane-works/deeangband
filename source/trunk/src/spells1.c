	...
	switch (typ)
	{
		...
		case GF_KILL_TRAP:
		{
			...
			if (is_trap(c_ptr->feat))
			{
				...
				/* Destroy the trap */
				cave_alter_feat(player_ptr, y, x, FF_DISARM);
			}
		...
		case GF_KILL_DOOR:
		{
			...
			if (is_trap(c_ptr->feat) || have_flag(f_ptr->flags, FF_DOOR))
			{
				...
				/* Destroy the feature */
				cave_alter_feat(player_ptr, y, x, FF_TUNNEL);
			}
		...
		case GF_JAM_DOOR: /* Jams a door (as if with a spike) */
		{
			if (have_flag(f_ptr->flags, FF_SPIKE))
			{
				...
				cave_alter_feat(player_ptr, y, x, FF_SPIKE);
		...
		case GF_MAKE_DOOR:
		{
			...
			if (!cave_naked_bold(aimer_ptr, y, x)) break;
