...
#ifdef JP
	sprintf(Dummy, "åªç›ÇÃëÆê´ : %s", buf2);
#else
...
void report_magics(creature_type *creature_ptr)
{
	...
	for (k = 2, j = 0; j < i; j++)
	{
		/* Show the info */
#ifdef JP
		sprintf(Dummy, "Ç†Ç»ÇΩÇÕ%s%s", info[j],
#else
		...
		for (dx = -r; dx <= r; dx++)
		{
			...
			if(c_ptr->creature_idx) // Process creatures
			{
				...
				if(!has_trait(target_ptr, TRAIT_PASS_WALL) && !has_trait(target_ptr, TRAIT_KILL_WALL))
				{
					...
					{
#ifdef JP
						if(is_player(target_ptr)) msg_print("Ç†Ç»ÇΩÇÕÇ–Ç«Ç¢âˆâ‰ÇïâÇ¡ÇΩÅI");
#else
					...
					else // Destroy the grid, and push the player to safety
					{
						...
						case 1:
							{
#ifdef JP
								if(is_player(target_ptr)) msg_print("ç~ÇËíçÇÆä‚ÇÇ§Ç‹Ç≠îÇØÇΩÅI");
#else
						...
						case 2:
							{
#ifdef JP
								if(is_player(target_ptr)) msg_print("ä‚êŒÇ™Ç†Ç»ÇΩÇ…íºåÇÇµÇΩ!");
#else
						...
						case 3:
							{
#ifdef JP
								if(is_player(target_ptr)) msg_print("Ç†Ç»ÇΩÇÕè∞Ç∆ï«Ç∆ÇÃä‘Ç…ã≤Ç‹ÇÍÇƒÇµÇ‹Ç¡ÇΩÅI");
#else
...
void discharge_minion(creature_type *caster_ptr)
{
	...
	{
#ifdef JP
		if(!get_check("ñ{ìñÇ…ëSÉyÉbÉgÇîöîjÇµÇ‹Ç∑Ç©ÅH"))
#else
...
#ifdef JP
			msg_format("%sÇÕîöîjÇ≥ÇÍÇÈÇÃÇåôÇ™ÇËÅAèüéËÇ…é©ï™ÇÃê¢äEÇ÷Ç∆ãAÇ¡ÇΩÅB", target_name);
#else
...
bool activate_ty_curse(creature_type *creature_ptr, bool stop_ty, int *count)
{
	...
	do
	{
		...
		case 32: case 33:
			...
			{
#ifdef JP
				msg_print("é¸àÕÇÃãÛä‘Ç™òcÇÒÇæÅI");
#else
		...
		case 34:
#ifdef JP
			msg_print("ÉGÉlÉãÉMÅ[ÇÃÇ§ÇÀÇËÇä¥Ç∂ÇΩÅI");
#else
		...
		case 13: case 14: case 15: case 19: case 20:
			...
			*/
			{
#ifdef JP
				msg_print("í§ëúÇ…Ç»Ç¡ÇΩãCï™ÇæÅI");
#else
		...
		case 24:
#ifdef JP
			msg_print("ÇŸÇ¶ÅHéÑÇÕíNÅHÇ±Ç±Ç≈âΩÇµÇƒÇÈÅH");
#else
...
bool rush_attack(creature_type *creature_ptr, bool *mdeath)
{
	...
	for (i = 0; i < path_n; i++)
	{
		...
		if(!floor_ptr->cave[ny][nx].creature_idx)
		{
			...
			{
#ifdef JP
				msg_print("Ç±Ç±Ç…ÇÕì¸êgÇ≈ÇÕì¸ÇÍÇ»Ç¢ÅB");
#else
