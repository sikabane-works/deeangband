...
bool cast_chain(creature_type *caster_ptr, int typ, COODINATES range, int num, POWER power, TRAIT_ID trait_id, FLAGS_32 options)
{
	...
	for (k = 0; k < num; k++)
	{
		...
		cast_beam_aux(caster_ptr, y, x, typ, range, power, trait_id, options);
	}
	return TRUE;
}
...
void cast_ball_aux(creature_type *caster_ptr, COODINATES y, COODINATES x, int typ, POWER power, COODINATES rad, TRAIT_ID trait_id, FLAGS_32 options)
{
	/* Analyze the "dir" and the "target".  Hurt items on floor. */
	(void)project(caster_ptr, 0, rad, y, x, power, typ, PROJECT_STOP | PROJECT_GRID | PROJECT_ITEM | PROJECT_KILL | options, trait_id);
}
...
bool cast_swarm(creature_type *caster_ptr, EFFECT_ID typ, COODINATES range, COODINATES rad, QUANTITY num, POWER power, TRAIT_ID trait_id, FLAGS_32 options)
{
	COODINATES x = caster_ptr->fx, y = caster_ptr->fy;
	int attempts, k;
	floor_type *floor_ptr = GET_FLOOR_PTR(caster_ptr);
	for (k = 0; k < num; k++)
	{
		...
		cast_ball_aux(caster_ptr, y, x, typ, power, rad, trait_id, options);
	}
	return TRUE;
}
