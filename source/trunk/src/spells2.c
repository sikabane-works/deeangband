...
bool genocide_aux(creature_type *user_ptr, int m_idx, int power, bool player_cast, int dam_side, cptr spell_name)
{
	...
	else if ((inside_quest && !random_quest_number(current_floor_ptr->dun_level)) || inside_arena || gamble_arena_mode) resist = TRUE;
...
bool symbol_genocide(creature_type *caster_ptr, int power, bool player_cast)
{
	...
	if ((inside_quest && !random_quest_number(current_floor_ptr->dun_level)) || inside_arena || gamble_arena_mode)
	{
		return (FALSE);
	}
