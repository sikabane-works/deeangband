...
static bool cave_monster_teleportable_bold(creature_type *cr_ptr, int y, int x, u32b mode)
{
	...
	{
		if (!creature_can_cross_terrain(c_ptr->feat, &species_info[cr_ptr->species_idx], 0)) return FALSE;
	}
...
void teleport_away_followable(creature_type *cr_ptr)
{
	...
	if (old_ml && (old_cdis <= MAX_SIGHT) && !the_world && !monster_arena_mode && los(cr_ptr->fy, cr_ptr->fx, oldfy, oldfx))
	{
		bool follow = FALSE;
...
static bool vanish_dungeon(creature_type *cr_ptr)
{
	...
	for (y = 1; y < cur_hgt - 1; y++)
	{
		for (x = 1; x < cur_wid - 1; x++)
		{
			...
			if (have_flag(f_ptr->flags, FF_HURT_DISI)) cave_alter_feat(cr_ptr, y, x, FF_HURT_DISI);
		}
	}
