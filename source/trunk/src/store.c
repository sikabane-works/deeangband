...
static void store_delete(store_type *st_ptr)
{
	...
	if(PROB_PERCENT(50)) num = (num + 1) / 2; /* Hack -- sometimes, only destroy half the items */
	if(PROB_PERCENT(50)) num = 1; /* Hack -- sometimes, only destroy a single item */
...
static int store_replacement(store_type *st_ptr, int num)
{
	...
	for (tries = 0; tries < num; tries++)
	{
		...
		apply_magic(find_unique_instance(st_ptr->owner_id), quest_ptr, level, AM_NO_FIXED_ART);
		...
		if(!flag)
		{
			...
			if(cur_ask > final_ask)
			{
				...
				if(annoyed > 3)
				{
					flag = TRUE;
#ifdef JP
					cancel = TRUE; /* 追加 $0 で買い取られてしまうのを防止 By FIRST*/
#endif
					(void)(increase_insults(st_ptr));
				}
			}
