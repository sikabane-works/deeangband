			...
			while (!done)
			{
				for (ty = y - width - 1; ty <= y + width + 1; ty++)
				{
					for (tx = x - width - 1; tx <= x + width + 1; tx++)
					{
						...
						{
							if (!(dungeon_info[dungeon_type].flags1 & DF1_DARKNESS)) c_ptr->info |= CAVE_GLOW;
						}
...
void build_streamer(int feat, int chance)
{
	...
	while (dummy < SAFE_MAX_ATTEMPTS)
	{
		...
		for (i = 0; i < DUN_STR_DEN; i++)
		{
			...
			if (c_ptr->m_idx && !(have_flag(streamer_ptr->flags, FF_PLACE) && creature_can_cross_terrain(feat, &species_info[creature_list[c_ptr->m_idx].species_idx], 0)))
			{
			...
			if (streamer_may_have_gold)
			{
				...
				if (one_in_(chance))
				{
					cave_alter_feat(p_ptr, ty, tx, FF_MAY_HAVE_GOLD);
				}
				...
				{
					cave_alter_feat(p_ptr, ty, tx, FF_MAY_HAVE_GOLD);
					cave_alter_feat(p_ptr, ty, tx, FF_ENSECRET);
				}
			}
		}
