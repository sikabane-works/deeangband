...
static void do_cmd_wiz_change_aux(creature_type *creature_ptr)
{
	...
	*/
	...
	for (j = 0; j < 32; j++)
		creature_ptr->spell_exp_old[j] = (tmp_s16b > SPELL_EXP_MASTER ? SPELL_EXP_MASTER : tmp_s16b);
	for (; j < (REALM_MAGIC_NUMBER * 2); j++)
		creature_ptr->spell_exp_old[j] = (tmp_s16b > SPELL_EXP_EXPERT ? SPELL_EXP_EXPERT : tmp_s16b);
...
static void wiz_display_item(object_type *object_ptr)
{
	...
	prt(format("xtra3 = %-4d  xtra4 = %-4d  cursed  = %-d",
		   object_ptr->xtra3, object_ptr->xtra4, object_ptr->curse_flags[0]), 9, j);
