...
cptr get_floor_name(floor_type *floor_ptr)
{
	if(floor_ptr->quest && is_fixed_quest_idx(floor_ptr->quest) && (quest[floor_ptr->quest].flags & QUEST_FLAG_PRESET))
		return KW_QUEST;
#ifdef JP
	else if(floor_ptr->global_map) return "ç¨ì◊ÇÃínïΩ";
#else
...
#ifdef JP
	if(s > 100) c_put_str(TERM_RED, "à”éØïsñæóƒ  ", ROW_STUN, COL_STUN);
	else if(s > 50) c_put_str(TERM_ORANGE, "Ç–Ç«Ç≠ûNûO  ", ROW_STUN, COL_STUN);
	else if(s) c_put_str(TERM_ORANGE, "ûNûO        ", ROW_STUN, COL_STUN);
#else 
