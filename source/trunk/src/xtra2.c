...
void creature_dead_effect(creature_type *slayer_ptr, creature_type *dead_ptr, bool drop_item)
{
	...
	for (j = 0; j < number; j++) /* Drop some objects */
	{
		...
		if(do_gold && (!do_item || (PROB_PERCENT(50))))/* Make Gold */
		{
...
static int target_set_aux(creature_type *creature_ptr, COODINATES y, COODINATES x, int mode, cptr info)
{
	...
	if(boring || have_flag(f_ptr->flags, FF_REMEMBER))
	{
		...
		else
		{
			...
			else if(floor_ptr->global_map && (feat == feat_floor)) name = KW_ROAD;
...
bool get_rep_dir(creature_type *creature_ptr, DIRECTION *dp, bool under)
{
	...
	else if(creature_ptr->riding)
	{
		...
		else if(has_trait_species(species_ptr, TRAIT_RAND_25) && has_trait_species(species_ptr, TRAIT_RAND_50) && (PROB_PERCENT(50)))
		{
			/* Random direction */
			dir = ddd[randint0(8)];
		}
		else if(has_trait_species(species_ptr, TRAIT_RAND_25) && (PROB_PERCENT(25)))
		{
			/* Random direction */
			dir = ddd[randint0(8)];
		}
...
{
	if (weight > 100000000000000) sprintf(buf, "%dMt", weight / 1000000000000);
	else if(weight > 10000000000000) sprintf(buf, "%d.%01dMt", weight / 1000000000, (weight % 1000000000) / 100000000);
	else if(weight > 1000000000000) sprintf(buf, "%d.%02dMt", weight / 1000000000, (weight % 1000000000) / 10000000);
	else if(weight > 100000000000) sprintf(buf, "%dMt", weight / 1000000000);
	else if(weight > 1000000000) sprintf(buf, "%d.%01dkt", weight / 1000000, (weight % 1000000) / 100000);
	else if(weight > 100000000) sprintf(buf, "%d.%02dkt", weight / 1000000, (weight % 1000000) / 10000);
	else if(weight > 10000000) sprintf(buf, "%dt", weight / 1000);
	else if(weight > 100000) sprintf(buf, "%d.%01dt", weight / 1000, (weight % 1000) / 100);
	else if(weight > 10000) sprintf(buf, "%d.%02dt", weight / 1000, (weight % 1000) / 10);
	else sprintf(buf, "%d.%1dkg", weight / 10, weight % 10);
	return;
}
