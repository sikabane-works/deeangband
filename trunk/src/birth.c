...
static byte get_intelligent_race_category(void)
{
	...
	/* Random Select*/
		...
		put_str("の種族修正", 3, 40+strlen(race_info[p_ptr->irace_idx].title));
		...
		if (cs != os)
		{
			...
			else
			{
				...
				if(race_info[p_ptr->irace_idx].dr >= 0)
					...
					sprintf(cur, "%c%c %s", sym[cs], p2, str);
				c_put_str(TERM_L_BLUE, race_info[p_ptr->irace_idx].title, 3, 40);
				put_str("の主種族修正", 3, 40+strlen(race_info[p_ptr->irace_idx].title));
				put_str("腕力 知能 賢さ 器用 耐久 魅力 経験   ", 4, 40);
#else
				if(race_info[p_ptr->irace_idx].dr >= 0)
					sprintf(cur, "%c%c!%s", sym[cs], p2, str);
		...
		if (cs != os)
		{
			...
			else
			{
				...
				if (!(race_info[p_ptr->irace_idx].choice & (1L << cs)))
					...
					put_str("の職業修正", 3, 40+strlen(class_info[p_ptr->cls_idx].title));
					put_str("腕力 知能 賢さ 器用 耐久 魅力 経験 ", 4, 40);
#else
					c_put_str(TERM_L_BLUE, class_info[p_ptr->cls_idx].title, 3, 40);
		...
		if (cs != os)
		{
			...
			else
			{
				str = chara_info[p_ptr->chara_idx].title;
					...
					put_str("の性格修正", 3, 40+strlen(chara_info[p_ptr->chara_idx].title));
