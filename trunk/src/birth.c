...
"秩序竜──ロードラゴンは、生まれつき〈秩序〉の力の体現者であり、恐ろしい音響と粒子の入り混じった息を吐きます。彼ら宇宙の根源に連なる種は、成長のためにより多くの経験を必要とします。",
"天上竜──イセリアルドラゴンは、天上界に拠り所を持つ強大な竜の種族であり、光と闇の力を合わせ持っています。彼ら完全なる世界の種は、成長のためにより多くの経験を必要とします。",
"混沌竜──カオスドラゴンは、生まれつき〈混沌〉の力を体現者であり、その吐息はあらゆるものを歪め、朽ち果てさせる力を宿しています。彼ら宇宙の根源に連なる種は、成長のためにより多くの経験を必要とします。",
...
static byte get_intelligent_race_category(void)
{
...
#ifdef JP
		sprintf(buf, "種族の系統を選んで下さい (%c-%c) ('='初期オプション設定): ", sym[0], sym[MAX_RACE_CATEGORYS-1]);
#else
	...
	/* Random Select*/
		...
		put_str("の種族修正", 3, 40+strlen(race_info[p_ptr->race].title));
	...
	while (1)
	{
		...
		if (cs != os)
		{
			...
			if(cs == c_num)
			{
				...
				sprintf(cur, "%c%c %s", '*', p2, "Random");
#endif
				put_str("この種族系統からランダムに選択します ", 4, 40);
				put_str("                                     ", 5, 40);
			}
			else
			{
				...
				put_str("の主種族修正", 3, 40+strlen(rp_ptr->title));
				put_str("腕力 知能 賢さ 器用 耐久 魅力 経験   ", 4, 40);
#else
				if(rp_ptr->dr >= 0)
					sprintf(cur, "%c%c!%s", sym[cs], p2, str);
...
#ifdef JP
	sprintf(buf,"                            種族に%sを選択した。", race_info[p_ptr->race].title);
#else
...
#ifdef JP
	sprintf(buf,"                            職業に%sを選択した。", class_info[p_ptr->class].title);
#else
