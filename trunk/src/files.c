...
static void player_flags(u32b flgs[TR_FLAG_SIZE], creature_type *cr_ptr)
{
	...
	if (cr_ptr->muta3)
	{
		...
		if ((cr_ptr->muta3 & MUT3_XTRA_FAT) ||
			(cr_ptr->muta3 & MUT3_XTRA_LEGS) ||
			(cr_ptr->muta3 & MUT3_SHORT_LEG))
		{
			add_flag(flgs, TR_SPEED);
		}
...
static void player_vuln_flags(u32b flgs[TR_FLAG_SIZE], creature_type *cr_ptr)
{
	...
	if ((cr_ptr->muta3 & MUT3_VULN_ELEM) || (cr_ptr->special_defense & KATA_KOUKIJIN))
	{
		add_flag(flgs, TR_RES_ACID);
		add_flag(flgs, TR_RES_ELEC);
		add_flag(flgs, TR_RES_FIRE);
...
static void display_player_other_flag_info(creature_type *cr_ptr)
{
	...
	display_flag_aux(row+ 1, col, "善良 倍打 :", TR_SLAY_GOOD, &f, DP_WP);
	display_flag_aux(row+ 1, col, "善良 倍打 :", TR_KILL_GOOD, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 2, col, "不死 倍打 :", TR_SLAY_UNDEAD, &f, DP_WP);
	display_flag_aux(row+ 2, col, "不死 倍打 :", TR_KILL_UNDEAD, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 3, col, "悪魔 倍打 :", TR_SLAY_DEMON, &f, DP_WP);
	display_flag_aux(row+ 3, col, "悪魔 倍打 :", TR_KILL_DEMON, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 4, col, "龍 倍打   :", TR_SLAY_DRAGON, &f, DP_WP);
	display_flag_aux(row+ 4, col, "龍 倍打   :", TR_KILL_DRAGON, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 5, col, "人間 倍打 :", TR_SLAY_HUMAN, &f, DP_WP);
	display_flag_aux(row+ 5, col, "人間 倍打 :", TR_KILL_HUMAN, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 6, col, "動物 倍打 :", TR_SLAY_ANIMAL, &f, DP_WP);
	display_flag_aux(row+ 6, col, "動物 倍打 :", TR_KILL_ANIMAL, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 7, col, "オーク倍打:", TR_SLAY_ORC, &f, DP_WP);
	display_flag_aux(row+ 7, col, "オーク倍打:", TR_KILL_ORC, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 8, col, "トロル倍打:", TR_SLAY_TROLL, &f, DP_WP);
	display_flag_aux(row+ 8, col, "トロル倍打:", TR_KILL_TROLL, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+ 9, col, "巨人 倍打 :", TR_SLAY_GIANT, &f, DP_WP);
	display_flag_aux(row+ 9, col, "巨人 倍打 :", TR_KILL_GIANT, &f, (DP_WP|DP_IMM));
	display_flag_aux(row+10, col, "溶解      :", TR_BRAND_ACID, &f, DP_WP);
	display_flag_aux(row+11, col, "電撃      :", TR_BRAND_ELEC, &f, DP_WP);
	display_flag_aux(row+12, col, "焼棄      :", TR_BRAND_FIRE, &f, DP_WP);
	display_flag_aux(row+13, col, "凍結      :", TR_BRAND_COLD, &f, DP_WP);
	display_flag_aux(row+14, col, "毒殺      :", TR_BRAND_POIS, &f, DP_WP);
	display_flag_aux(row+15, col, "切れ味    :", TR_VORPAL, &f, DP_WP);
	display_flag_aux(row+16, col, "地震      :", TR_IMPACT, &f, DP_WP);
	display_flag_aux(row+17, col, "吸血      :", TR_VAMPIRIC, &f, DP_WP);
	display_flag_aux(row+18, col, "カオス効果:", TR_CHAOTIC, &f, DP_WP);
	display_flag_aux(row+19, col, "理力      :", TR_FORCE_WEAPON, &f, DP_WP);
#else
