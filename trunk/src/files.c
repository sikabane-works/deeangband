...
void display_player(int mode, creature_type *cr_ptr)
{
	...
	if ((mode == 0) || (mode == 1))
	{
		...
		if(cr_ptr->dr < 0){
			...
			display_player_one_line(ENTRY_DIVINE_RANK, format("究極神[%2d]", cr_ptr->dr), TERM_WHITE);
		else if(cr_ptr->dr >= 21)
			display_player_one_line(ENTRY_DIVINE_RANK, format("超越神[%2d]", cr_ptr->dr), TERM_VIOLET);
