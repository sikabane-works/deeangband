...
static void display_player_other_flag_info(creature_type *cr_ptr)
{
	...
	display_flag_aux(row+10, col, "溶解      :", TR_BRAND_ACID, &f, DP_WP, cr_ptr);
	display_flag_aux(row+11, col, "電撃      :", TR_BRAND_ELEC, &f, DP_WP, cr_ptr);
	display_flag_aux(row+12, col, "焼棄      :", TR_BRAND_FIRE, &f, DP_WP, cr_ptr);
	display_flag_aux(row+13, col, "凍結      :", TR_BRAND_COLD, &f, DP_WP, cr_ptr);
	display_flag_aux(row+14, col, "毒殺      :", TR_BRAND_POIS, &f, DP_WP, cr_ptr);
	display_flag_aux(row+15, col, "切れ味    :", TR_VORPAL, &f, DP_WP, cr_ptr);
	display_flag_aux(row+16, col, "地震      :", TR_IMPACT, &f, DP_WP, cr_ptr);
	display_flag_aux(row+17, col, "吸血      :", TR_VAMPIRIC, &f, DP_WP, cr_ptr);
	display_flag_aux(row+18, col, "カオス効果:", TR_CHAOTIC, &f, DP_WP, cr_ptr);
	display_flag_aux(row+19, col, "理力      :", TR_FORCE_WEAPON, &f, DP_WP, cr_ptr);
#else
...
void display_player(int mode, creature_type *cr_ptr)
{
	...
	if ((mode == 0) || (mode == 1))
	{
		...
		if(cr_ptr->chara_idx != CHARA_NONE){ 
#ifdef JP
			sprintf(tmp, "%s%s%s", chara_info[p_ptr->chara_idx].title, chara_info[p_ptr->chara_idx].no == 1 ? "の":"", cr_ptr->name);
#else
