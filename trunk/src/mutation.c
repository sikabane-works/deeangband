...
muta_desc = "関節が突然痛み出した。";
			...
			break;
		case 188:
			if (p_ptr->seikaku == SEIKAKU_LUCKY) break;
			muta_class = &(p_ptr->muta3);
...
muta_desc = "声質がかなり強くなった。";
			...
			else if (p_ptr->race == RACE_BEASTMAN &&
...
muta_desc = "あなたの肉体は変化できるようになった、";
			...
			else if (p_ptr->race == RACE_MIND_FLAYER &&
...
int calc_mutant_regenerate_mod(void)
{
	...
	if (p_ptr->seikaku == SEIKAKU_LUCKY) count--;
	if (p_ptr->race == RACE_BEASTMAN)
	{
		count -= 10;
		mod = 5;
	}
