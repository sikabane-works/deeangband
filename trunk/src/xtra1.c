...
static void prt_exp(void)
{
	...
	{
#ifdef JP
		if (p_ptr->irace_idx == RACE_ANDROID) put_str("強化 ", ROW_EXP, 0);
...
static void calc_spells(creature_type *cr_ptr, bool message)
{
	...
	if ((cr_ptr->class == CLASS_SORCERER) || (cr_ptr->class == CLASS_RED_MAGE))
	{
		cr_ptr->new_spells = 0;
		return;
	}
	...
	num_allowed = (adj_mag_study[cr_ptr->stat_ind[mp_ptr->spell_stat]] * levels / 2);

	if ((cr_ptr->class != CLASS_SAMURAI) && (mp_ptr->spell_book != TV_LIFE_BOOK))
	{
		bonus = 4;
	}
	...
	else if ((cr_ptr->class == CLASS_MAGE) || (cr_ptr->class == CLASS_PRIEST))
	{
	...
	for (i = 63; i >= 0; i--)
	{
		...
		if (!is_magic((j < 32) ? cr_ptr->realm1 : cr_ptr->realm2))
		{
			...
			s_ptr = &mp_ptr->info[cr_ptr->realm1-1][j];
		else
			s_ptr = &mp_ptr->info[cr_ptr->realm2-1][j%32];
	...
	if (cr_ptr->realm2 == REALM_NONE)
	{
		...
		for (j = 0; j < 32; j++)
		{
			...
			else s_ptr = &mp_ptr->info[cr_ptr->realm1-1][j];
		...
		if (k>32) k = 32;
		if ((cr_ptr->new_spells > k) && ((mp_ptr->spell_book == TV_LIFE_BOOK) || (mp_ptr->spell_book == TV_HISSATSU_BOOK))) cr_ptr->new_spells = k;
	}
...
void calc_bonuses(creature_type *cr_ptr, bool message)
{
	...
	if(cr_ptr->class != CLASS_NONE)
	{
		...
		cr_ptr->skill_rob += class_info[cr_ptr->class].c_sav;
		cr_ptr->skill_agi += class_info[cr_ptr->class].c_sav;
		cr_ptr->skill_vol += class_info[cr_ptr->class].c_sav;
	...
	if(cr_ptr->class != CLASS_NONE) cr_ptr->skill_rob += (class_info[cr_ptr->class].x_sav * cr_ptr->lev / 10);
	if(cr_ptr->class != CLASS_NONE) cr_ptr->skill_agi += (class_info[cr_ptr->class].x_sav * cr_ptr->lev / 10);
	if(cr_ptr->class != CLASS_NONE) cr_ptr->skill_vol += (class_info[cr_ptr->class].x_sav * cr_ptr->lev / 10);
...
s16b empty_hands(bool riding_control)
{
	...
	if (!p_ptr->inventory[INVEN_RARM].k_idx) status |= EMPTY_HAND_RARM;
	if (!p_ptr->inventory[INVEN_LARM].k_idx) status |= EMPTY_HAND_LARM;
