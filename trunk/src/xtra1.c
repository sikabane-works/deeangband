...
static void prt_title(void)
{
	...
	{
		my_strcpy(str, player_title[p_ptr->class][(p_ptr->lev - 1) / 6], sizeof(str));
		p = str;
	}
...
static void prt_exp(void)
{
	...
	{
#ifdef JP
		if (p_ptr->race == RACE_ANDROID) put_str("強化 ", ROW_EXP, 0);
...
static void calc_spells(void)
{
	...
	if ((p_ptr->class != CLASS_SAMURAI) && (mp_ptr->spell_book != TV_LIFE_BOOK))
	{
		bonus = 4;
	}
	...
	else if ((p_ptr->class == CLASS_MAGE) || (p_ptr->class == CLASS_PRIEST))
	{
...
static void calc_mana(void)
{
	...
	else
	{
		...
		if (msp && (p_ptr->seikaku == SEIKAKU_MUNCHKIN)) msp += msp/2;
...
u32b weight_limit(void)
{
	...
	if (p_ptr->class == CLASS_BERSERKER) i = i * 3 / 2;
...
void calc_bonuses(creature_type *cr_ptr)
{
	...
	for(i = 0; i < MAX_RACES; i++)
	{
		if(get_subrace(i)){
			...
			cr_ptr->skill_sav += race_info[i].r_s_sav;
	...
	switch (cr_ptr->class)
	{
		...
		case CLASS_FORCETRAINER:
			...
			if (!(heavy_armor()))
			{
				if (!(prace_is_(RACE_KLACKON) ||
				      ...
				      (cr_ptr->seikaku == SEIKAKU_MUNCHKIN)))
		...
		case CLASS_NINJA:
			...
			else if ((!inventory[INVEN_RARM].k_idx || cr_ptr->migite) &&
			         (!inventory[INVEN_LARM].k_idx || cr_ptr->hidarite))
			{
				new_speed += 3;
				if (!(prace_is_(RACE_KLACKON) ||
				      prace_is_(RACE_SPRITE) ||
			...
			if ((!inventory[INVEN_RARM].k_idx || cr_ptr->migite) &&
			    (!inventory[INVEN_LARM].k_idx || cr_ptr->hidarite))
			{
	...
	/* Sexy Gal */
	if (cr_ptr->seikaku == SEIKAKU_SEXY) cr_ptr->cursed |= (TRC_AGGRAVATE);
	if (cr_ptr->seikaku == SEIKAKU_NAMAKE) cr_ptr->to_m_chance += 10;
	if (cr_ptr->seikaku == SEIKAKU_KIREMONO) cr_ptr->to_m_chance -= 3;
	if ((cr_ptr->seikaku == SEIKAKU_GAMAN) || (cr_ptr->seikaku == SEIKAKU_CHIKARA)) cr_ptr->to_m_chance++;

	/* Lucky man */
	if (cr_ptr->seikaku == SEIKAKU_LUCKY) cr_ptr->muta3 |= MUT3_GOOD_LUCK;

	if (cr_ptr->seikaku == SEIKAKU_MUNCHKIN)
	{
		cr_ptr->resist_blind = TRUE;
		cr_ptr->resist_conf  = TRUE;
		cr_ptr->hold_life = TRUE;
	...
	if (cr_ptr->muta3)
	{
		...
		{
			cr_ptr->skill_sav += (15 + (cr_ptr->lev / 5));
		}
	...
	for (i = INVEN_RARM; i < INVEN_TOTAL; i++)
	{
		...
		/* Affect speed */
		if(i == INVEN_BODY || i == INVEN_HEAD || i ==  INVEN_HANDS || i == INVEN_FEET)
		{
		...
		{
			if((i == INVEN_RARM || i == INVEN_RIGHT) && !cr_ptr->ryoute) extra_blows[0] += o_ptr->pval;
			else if((i == INVEN_LARM || i == INVEN_LEFT) && !cr_ptr->ryoute) extra_blows[1] += o_ptr->pval;
			else {extra_blows[0] += o_ptr->pval; extra_blows[1] += o_ptr->pval;}
		}

		/* Hack -- cause earthquakes */
		if (have_flag(flgs, TR_IMPACT)) cr_ptr->impact[(i == INVEN_RARM) ? 0 : 1] = TRUE;
	...
	if (((cr_ptr->class == CLASS_MONK) || (cr_ptr->class == CLASS_FORCETRAINER)) && !heavy_armor())
	{
		...
		if (!(inventory[INVEN_OUTER].k_idx) && (cr_ptr->lev > 15))
		{
			cr_ptr->to_a += ((cr_ptr->lev - 13) / 3);
			cr_ptr->dis_to_a += ((cr_ptr->lev - 13) / 3);
		}
		if (!(inventory[INVEN_LARM].k_idx) && (cr_ptr->lev > 10))
		{
			cr_ptr->to_a += ((cr_ptr->lev - 8) / 3);
			cr_ptr->dis_to_a += ((cr_ptr->lev - 8) / 3);
		}
		if (!(inventory[INVEN_HEAD].k_idx) && (cr_ptr->lev > 4))
		{
	...
	for (i = 0; i < 6; i++)
	{
		...
		if ((i == A_CHR) && (cr_ptr->muta3 & MUT3_ILL_NORM))
		{
	...
	if (buki_motteruka(INVEN_RARM) && buki_motteruka(INVEN_LARM))
	{
		...
		penalty1 = ((100 - cr_ptr->skill_exp[GINOU_NITOURYU] / 160) - (130 - inventory[INVEN_RARM].weight) / 8);
		penalty2 = ((100 - cr_ptr->skill_exp[GINOU_NITOURYU] / 160) - (130 - inventory[INVEN_LARM].weight) / 8);
	...
	if (o_ptr->k_idx)
	{
		...
		if (o_ptr->k_idx && !cr_ptr->heavy_shoot)
		{
			...
			if ((cr_ptr->class == CLASS_RANGER) &&
			    (cr_ptr->tval_ammo == TV_ARROW))
			{
				cr_ptr->num_fire += (cr_ptr->lev * 4);
			}

			if ((cr_ptr->class == CLASS_CAVALRY) &&
			    (cr_ptr->tval_ammo == TV_ARROW))
			{
			 ...
			 */
			if (cr_ptr->class == CLASS_WARRIOR &&
			   (cr_ptr->tval_ammo <= TV_BOLT) &&
			   (cr_ptr->tval_ammo >= TV_SHOT))
			{
				cr_ptr->num_fire += (cr_ptr->lev * 2);
			}
			if ((cr_ptr->class == CLASS_ROGUE) &&
			    (cr_ptr->tval_ammo == TV_SHOT))
			{
				cr_ptr->num_fire += (cr_ptr->lev * 4);
			}
	...
	for(i = 0 ; i < 2 ; i++)
	{
		...
		if (o_ptr->k_idx && !cr_ptr->heavy_wield[i])
		{
			...
			if (cr_ptr->class == CLASS_NINJA) str_index = MAX(0, str_index-1);
			...
			if (cr_ptr->class == CLASS_WARRIOR) cr_ptr->num_blow[i] += (cr_ptr->lev / 40);
			else if (cr_ptr->class == CLASS_BERSERKER)
			{
				cr_ptr->num_blow[i] += (cr_ptr->lev / 23);
			}
			else if ((cr_ptr->class == CLASS_ROGUE) && (o_ptr->weight < 50) && (cr_ptr->stat_ind[A_DEX] >= 30)) cr_ptr->num_blow[i] ++;
		...
		if ((cr_ptr->class == CLASS_PRIEST) && (!(have_flag(flgs, TR_BLESSED))) &&
		...
		else if (cr_ptr->class == CLASS_SORCERER)
		{
		...
		if (cr_ptr->riding)
		{
			...
			{
				int penalty;
				if ((cr_ptr->class == CLASS_BEASTMASTER) || (cr_ptr->class == CLASS_CAVALRY))
				{
					penalty = 5;
				}
				else
	...
	if (cr_ptr->riding)
	{
		...
		if (cr_ptr->ryoute || (empty_hands(FALSE) == EMPTY_HAND_NONE)) cr_ptr->riding_ryoute = TRUE;
	...
	if (((cr_ptr->class == CLASS_MONK) || (cr_ptr->class == CLASS_FORCETRAINER) || (cr_ptr->class == CLASS_BERSERKER)) &&
		(empty_hands_status & EMPTY_HAND_RARM) && !cr_ptr->hidarite)
	{
		int blow_base = cr_ptr->lev + adj_dex_blow[cr_ptr->stat_ind[A_DEX]];
		cr_ptr->num_blow[0] = 0;

		if (cr_ptr->class == CLASS_FORCETRAINER)
		{
		...
		if (heavy_armor() && (cr_ptr->class != CLASS_BERSERKER))
			cr_ptr->num_blow[0] /= 2;
		else
		{
		...
		else if (cr_ptr->special_defense & KAMAE_GENBU)
		{
			...
			cr_ptr->reflect = TRUE;
			cr_ptr->num_blow[0] -= 2;
			if ((cr_ptr->class == CLASS_MONK) && (cr_ptr->lev > 42)) cr_ptr->num_blow[0]--;
	...
	for (i = 0; i < 2; i++)
	{
		if (buki_motteruka(INVEN_RARM+i))
		{
			...
			if ((cr_ptr->class == CLASS_MONK) || (cr_ptr->class == CLASS_FORCETRAINER))
			{
				if (!s_info[cr_ptr->class].w_max[tval][sval])
				{
					cr_ptr->to_h[i] -= 40;
			...
			if (inventory[INVEN_RARM + i].name1 == ART_IRON_BALL) cr_ptr->align -= 1000;
		}
	}
	...
	if (((cr_ptr->class == CLASS_MONK) || (cr_ptr->class == CLASS_FORCETRAINER) || (cr_ptr->class == CLASS_BERSERKER)) && (empty_hands(FALSE) == (EMPTY_HAND_RARM | EMPTY_HAND_LARM))) cr_ptr->ryoute = FALSE;
	...
	cr_ptr->skill_sav += adj_wis_sav[cr_ptr->stat_ind[A_WIS]];
	...
	if ((prace_is_(RACE_S_FAIRY)) && (cr_ptr->seikaku != SEIKAKU_SEXY) && (cr_ptr->cursed & TRC_AGGRAVATE))
	{
	...
	if (cr_ptr->anti_magic && (cr_ptr->skill_sav < (90 + cr_ptr->lev))) cr_ptr->skill_sav = 90 + cr_ptr->lev;

	if (cr_ptr->tsubureru) cr_ptr->skill_sav = 10;

	if ((cr_ptr->ult_res || cr_ptr->resist_magic || cr_ptr->magicdef) && (cr_ptr->skill_sav < (95 + cr_ptr->lev))) cr_ptr->skill_sav = 95 + cr_ptr->lev;

	if (down_saving) cr_ptr->skill_sav /= 2;
	...
	if (((cr_ptr->class == CLASS_MONK) || (cr_ptr->class == CLASS_FORCETRAINER) || (cr_ptr->class == CLASS_NINJA)) && (monk_armour_aux != monk_notify_aux))
	{
		if (heavy_armor())
		{
#ifdef JP
...
#if 0
	if (have_dd_s && ((cr_ptr->realm1 == REALM_SORCERY) || (cr_ptr->realm2 == REALM_SORCERY) || (cr_ptr->class == CLASS_SORCERER)))
	{
		magic_type *s_ptr = &mp_ptr->info[REALM_SORCERY-1][SPELL_DD_S];
		if (cr_ptr->lev >= s_ptr->slevel) cr_ptr->no_flowed = TRUE;
	}

	if (have_dd_t && ((cr_ptr->realm1 == REALM_TRUMP) || (cr_ptr->realm2 == REALM_TRUMP) || (cr_ptr->class == CLASS_SORCERER) || (cr_ptr->class == CLASS_RED_MAGE)))
	{
		magic_type *s_ptr = &mp_ptr->info[REALM_TRUMP-1][SPELL_DD_T];
		if (cr_ptr->lev >= s_ptr->slevel) cr_ptr->no_flowed = TRUE;
	}
#endif
	if (have_sw && ((cr_ptr->realm1 == REALM_NATURE) || (cr_ptr->realm2 == REALM_NATURE) || (cr_ptr->class == CLASS_SORCERER)))
	{
		magic_type *s_ptr = &mp_ptr->info[REALM_NATURE-1][SPELL_SW];
		if (cr_ptr->lev >= s_ptr->slevel) cr_ptr->no_flowed = TRUE;
	}

	if (have_kabe && ((cr_ptr->realm1 == REALM_CRAFT) || (cr_ptr->realm2 == REALM_CRAFT) || (cr_ptr->class == CLASS_SORCERER)))
	{
...
bool heavy_armor(void)
{
	...
	if ((p_ptr->class != CLASS_MONK) && (p_ptr->class != CLASS_FORCETRAINER) && (p_ptr->class != CLASS_NINJA)) return FALSE;
