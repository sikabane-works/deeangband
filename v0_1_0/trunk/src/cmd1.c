		...
		case TRAP_ARMAGEDDON:
		{
			...
			for (lev = dun_level; lev >= 20; lev -= 1 + lev/16)
			{
				...
				for (i = 0; i < num; i++)
				{
					...
					if (!projectable(py, p_ptr->fx, y1, x1)) continue;
...
#ifdef JP
				take_hit(NULL, cr_ptr, DAMAGE_FORCE, k, "死の大鎌", NULL, -1);
#else
...
bool py_attack(creature_type *atk_ptr, int y, int x, int mode)
{
	...
	/* Player or Enemy */
	if(p_ptr->fx == x && py == y && c_ptr->m_idx)
	{
		if(one_in_(2))
			tar_ptr = p_ptr;
		else
	    ...
	    atk_ptr->shero || !tar_ptr->ml))
	{
		...
		{
#ifdef JP
			msg_format("黒い刃は強欲に%sを攻撃した！", tar_name);
#else
		...
		else if (atk_ptr->cls_idx != CLASS_BERSERKER && atk_ptr == p_ptr)
		{
			...
			{
#ifdef JP
				msg_format("%sを攻撃するのを止めた。", tar_name);
#else
	...
	if (atk_ptr->afraid)
	{
		...
		if (tar_ptr->ml)
#ifdef JP
			msg_format("%sは怯えていて%sを攻撃できない！", atk_name, tar_name);
#else
	...
	if(!mdeath)
	{
		...
		if (atk_ptr->size > tar_ptr->size && randint0(100) < prob)
		{
			int k;
#ifdef JP
			msg_format("%sは残酷にも%sを踏みつけた！", atk_name, tar_name);
#else
	...
	if (fear && tar_ptr->ml && !mdeath)
	{
		...
		/* Message */
#ifdef JP
		msg_format("%^sは恐怖して逃げ出した！", tar_name);
#else
...
bool move_player_effect(int ny, int nx, u32b mpe_mode)
{
	...
	if (mpe_mode & MPE_ENERGY_USE)
	{
		...
		{
			(void)project(0, 0, py, p_ptr->fx, (60 + p_ptr->lev), GF_DISINTEGRATE,
...
void move_player(int dir, bool do_pickup, bool break_trap)
{
	...
	if (c_ptr->m_idx && (m_ptr->ml || p_can_enter || p_can_kill_walls))
	{
		...
		if (!is_hostile(m_ptr) &&
		    ...
		    pattern_seq(py, p_ptr->fx, y, x) && (p_can_enter || p_can_kill_walls))
		{
			/* Disturb the monster */
			(void)set_paralyzed(m_ptr, 0);
			...
			else if (monster_can_cross_terrain(cave[py][p_ptr->fx].feat, r_ptr, 0))
			{
				do_past = TRUE;
			}
			else
	...
	if (oktomove && p_ptr->riding)
	{
		...
		{
#ifdef JP
			msg_format("%sから上がれない。", f_name + f_info[get_feat_mimic(&cave[py][p_ptr->fx])].name);
#else
	...
	/* Normal movement */
	if (oktomove && !pattern_seq(py, p_ptr->fx, y, x))
	{
		if (!(p_ptr->confused || p_ptr->stun || p_ptr->image))
		{
			energy_use = 0;
...
static void run_init(int dir)
{
	...
	/* Check for walls */
	if (see_wall(cycle[i+1], py, p_ptr->fx))
	{
		find_breakleft = TRUE;
		shortleft = TRUE;
	}
...
static bool run_test(void)
{
	...
	if ((disturb_trap_detect || alert_trap_detect)
	    && p_ptr->dtrap && !(cave[py][p_ptr->fx].info & CAVE_IN_DETECT))
	{
		/* No duplicate warning */
		p_ptr->dtrap = FALSE;

		/* You are just on the edge */
		if (!(cave[py][p_ptr->fx].info & CAVE_UNSAFE))
		{
			if (alert_trap_detect)
			{
#ifdef JP
	...
	if (find_openarea)
	{
		...
		for (i = -max; i < 0; i++)
		{
			...
			if (!see_wall(cycle[chome[prev_dir] + i], py, p_ptr->fx))
			{
	...
	if (see_wall(find_current, py, p_ptr->fx))
	{
		return (TRUE);
	}
...
static bool travel_test(void)
{
	...
	for (i = 0; i < 8; i++)
	{
		if (travel.cost[py+ddy_ddd[i]][p_ptr->fx+ddx_ddd[i]] < travel.cost[py][p_ptr->fx]) stop = FALSE;
	}
...
void travel_step(void)
{
	...
	for (i = 1; i <= 9; i++)
	{
		...
		if (travel.cost[py+ddy[i]][p_ptr->fx+ddx[i]] < travel.cost[py+ddy[dir]][p_ptr->fx+ddx[dir]])
		{
			dir = i;
		}
	}
	...
	/* Close door */
	if (!easy_open && is_closed_door(cave[py+ddy[dir]][p_ptr->fx+ddx[dir]].feat))
	{
		disturb(0, 0);
		return;
	}
	...
	if ((py == travel.y) && (p_ptr->fx == travel.x))
		travel.run = 0;
	else
		travel.run--;
}
