			...
			if (touched)
			{
				if (p_ptr->sh_fire && alive && !p_ptr->is_dead)
				{
					if (!(r_ptr->flagsr & RFR_EFF_IM_FIRE_MASK))
					{
						...
						mon_take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕäDÇÃéRÇ…Ç»Ç¡ÇΩÅB", -1);
#else
				...
				if (p_ptr->sh_elec && alive && !p_ptr->is_dead)
				{
					if (!(r_ptr->flagsr & RFR_EFF_IM_ELEC_MASK))
					{
						...
						mon_take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕîRÇ¶äkÇÃéRÇ…Ç»Ç¡ÇΩÅB", -1);
#else
				...
				if (p_ptr->sh_cold && alive && !p_ptr->is_dead)
				{
					if (!(r_ptr->flagsr & RFR_EFF_IM_COLD_MASK))
					{
						...
						mon_take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕìÄÇËÇ¬Ç¢ÇΩÅB", -1);
#else
				...
				if (p_ptr->dustrobe && alive && !p_ptr->is_dead)
				{
					if (!(r_ptr->flagsr & RFR_EFF_RES_SHAR_MASK))
					{
						...
						mon_take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕÉYÉ^ÉYÉ^Ç…Ç»Ç¡ÇΩÅB", -1);
#else
						msg_format("%^s gets zapped!", m_name);
				...
				if (p_ptr->tim_sh_holy && alive && !p_ptr->is_dead)
				{
					if (r_ptr->flags3 & RF3_EVIL)
					{
						if (!(m_ptr->resist_ultimate))
						{
							...
							mon_take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕì|ÇÍÇΩÅB", -1);
#else
