...
bool make_attack_normal(int m_idx)
{
	...
	{
#ifdef JP
		msg_print("ëäéËÇ™èPÇ¢Ç©Ç©ÇÈëOÇ…ëfëÅÇ≠ïêäÌÇêUÇÈÇ¡ÇΩÅB");
#else
		...
		if (!effect || check_hit(power, rlev, m_ptr->stun))
		{
			    ...
			    ((randint0(100) + p_ptr->lev) > 50))
			{
				...
				if (abbreviate)
				    ...
				    msg_format("%^sÇÕåÇëﬁÇ≥ÇÍÇΩÅB", m_name);
			...
			switch (effect)
			{
				...
				case RBE_SUPERHURT:
				{
					if (((randint1(rlev*2+300) > (ac+200)) || one_in_(13)) && !(p_ptr->multishadow && (turn & 1)))
					{
						int tmp_damage = damage - (damage * ((ac < 150) ? ac : 150) / 250);
#ifdef JP
						msg_print("í…ç¶ÇÃàÍåÇÅI");
#else
				...
				case RBE_POISON:
				{
					...
					/* Take "poison" effect */
					if (!(p_ptr->resist_pois || IS_OPPOSE_POIS(p_ptr)) && !(p_ptr->multishadow && (turn & 1)))
					{
				...
				case RBE_UN_BONUS:
				{
					...
					if (!p_ptr->resist_disen && !(p_ptr->multishadow && (turn & 1)))
					{
				...
				case RBE_UN_POWER:
				{
					...
					if (p_ptr->is_dead || (p_ptr->multishadow && (turn & 1))) break;
					...
					for (k = 0; k < 10; k++)
					{
						    ...
						    (o_ptr->pval))
						{
							...
							/* Message */
#ifdef JP
							msg_print("ÉUÉbÉNÇ©ÇÁÉGÉlÉãÉMÅ[Ç™ãzÇ¢éÊÇÁÇÍÇΩÅI");
#else
				...
				case RBE_BLIND:
				{
					...
					/* Increase "blind" */
					if (!p_ptr->resist_blind && !(p_ptr->multishadow && (turn & 1)))
					{
						if (set_blind(p_ptr, p_ptr->blind + 10 + randint1(rlev)))
						{
#ifdef JP
							if (m_ptr->monster_idx == MON_DIO) msg_print("Ç«Ç§ÇæÉbÅIÇ±ÇÃååÇÃñ⁄í◊ÇµÇÕÉbÅI");
#else
							/* nanka */
#endif
							obvious = TRUE;
				...
				case RBE_CONFUSE:
				{
					...
					/* Increase "confused" */
					if (!p_ptr->resist_conf && !(p_ptr->multishadow && (turn & 1)))
					{
				...
				{
					if (explode) break;
					if (!p_ptr->resist_time && !(p_ptr->multishadow && (turn & 1)))
					{
						switch (randint1(10))
						{
				...
				case RBE_EXP_VAMP:
				{
					...
					if ((damage > 5) && !resist_drain)
					{
						...
						{
#ifdef JP
msg_format("%sÇÕëÃóÕÇâÒïúÇµÇΩÇÊÇ§ÇæÅB", m_name);
#else
...
#ifdef JP
						msg_format("%^sÇÕìÀëRîMÇ≠Ç»Ç¡ÇΩÅI", m_name);
						take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕäDÇÃéRÇ…Ç»Ç¡ÇΩÅB", -1);
#else
...
#ifdef JP
						msg_format("%^sÇÕìdåÇÇÇ≠ÇÁÇ¡ÇΩÅI", m_name);
						take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕîRÇ¶äkÇÃéRÇ…Ç»Ç¡ÇΩÅB", -1);
#else
...
#ifdef JP
						msg_format("%^sÇÕó‚ãCÇÇ≠ÇÁÇ¡ÇΩÅI", m_name);
						take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕìÄÇËÇ¬Ç¢ÇΩÅB", -1);
#else
...
#ifdef JP
						msg_format("%^sÇÕãæÇÃîjï–ÇÇ≠ÇÁÇ¡ÇΩÅI", m_name);
						take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕÉYÉ^ÉYÉ^Ç…Ç»Ç¡ÇΩÅB", -1);
#else
						msg_format("%^s gets zapped!", m_name);
...
#ifdef JP
							msg_format("%^sÇÕêπÇ»ÇÈÉIÅ[ÉâÇ≈èùÇ¬Ç¢ÇΩÅI", m_name);
							take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕì|ÇÍÇΩÅB", -1);
#else
...
#ifdef JP
						msg_format("%^sÇ™âsÇ¢ì¨ãCÇÃÉIÅ[ÉâÇ≈èùÇ¬Ç¢ÇΩÅI", m_name);
						take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕì|ÇÍÇΩÅB", -1);
#else
...
#ifdef JP
						msg_format("âeÇÃÉIÅ[ÉâÇ™%^sÇ…îΩåÇÇµÇΩÅI", m_name);
						take_hit(p_ptr, &m_list[m_idx], 0, dam, NULL, "ÇÕì|ÇÍÇΩÅB", -1);
#else
...
#ifdef JP
					if (abbreviate)
					    msg_format("%sÇ©ÇÌÇµÇΩÅB", (p_ptr->special_attack & ATTACK_SUIKEN) ? "äÔñ≠Ç»ìÆÇ´Ç≈" : "");
					else
					    msg_format("%s%^sÇÃçUåÇÇÇ©ÇÌÇµÇΩÅB", (p_ptr->special_attack & ATTACK_SUIKEN) ? "äÔñ≠Ç»ìÆÇ´Ç≈" : "", m_name);
					abbreviate = 1;/*ÇQâÒñ⁄à»ç~ÇÕè»ó™ */
#else
...
msg_format("%^sÇ©ÇÁóéÇøÇƒÇµÇ‹Ç¡ÇΩÅI", m_name);
	...
	{
#ifdef JP
		msg_format("çUåÇÇ™%sé©êgÇèùÇ¬ÇØÇΩÅI", m_name);
#else
		char m_name_self[80];
	...
	if ((p_ptr->counter || (p_ptr->special_defense & KATA_MUSOU)) && alive && !p_ptr->is_dead && m_ptr->ml && (p_ptr->csp > 7))
	{
		...
		p_ptr->csp -= 7;
#ifdef JP
		msg_format("%^sÇ…îΩåÇÇµÇΩÅI", m_name);
#else
