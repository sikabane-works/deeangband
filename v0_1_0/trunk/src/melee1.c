...
bool make_attack_normal(creature_type *atk_ptr, creature_type *tar_ptr)
{
	...
	for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
	{
		...
		if (is_pet(atk_ptr) && (r_ptr->flags1 & RF1_UNIQUE) && (method == RBM_EXPLODE))
		{
			method = RBM_HIT;
			d_dice /= 10;
		}
		...
		if (distance(py, px, atk_ptr->fy, atk_ptr->fx) > 1) break;
			...
			switch (effect)
			{
				...
				case RBE_EAT_GOLD:
				{
					...
					else
					{
						...
						else
						{
							...
							chg_virtue(tar_ptr, V_SACRIFICE, 2);
						}
				...
				case RBE_BLIND:
				{
					...
					if (!tar_ptr->resist_blind && !(tar_ptr->multishadow && (turn & 1)))
					{
						...
						{
#ifdef JP
							if (atk_ptr->monster_idx == MON_DIO) msg_print("Ç«Ç§ÇæÉbÅIÇ±ÇÃååÇÃñ⁄í◊ÇµÇÕÉbÅI");
#else
							/* nanka */
#endif
							obvious = TRUE;
				...
				case RBE_DISEASE:
				{
					...
					if ((randint1(100) < 11) && (tar_ptr->irace_idx != RACE_ANDROID))
					{
			...
			switch (method)
			{
				...
				if (atk_ptr->ml)
				{
					...
					if (abbreviate)
					    ...
					    msg_format("%s%^sÇÃçUåÇÇÇ©ÇÌÇµÇΩÅB", (tar_ptr->special_attack & ATTACK_SUIKEN) ? "äÔñ≠Ç»ìÆÇ´Ç≈" : "", m_name);
					abbreviate = 1;/*ÇQâÒñ⁄à»ç~ÇÕè»ó™ */
#else
