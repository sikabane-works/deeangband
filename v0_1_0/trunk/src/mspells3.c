...
msg_print("援軍を召喚した。");
		...
		for (k = 0;k < 1; k++)
		{
			if (summon_kin_player(summon_lev, py, p_ptr->fx, (pet ? PM_FORCE_PET : 0L)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0 ;k < 1 ; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_CYBER, p_mode))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, 0, p_mode))
			{
				if (!pet)
#ifdef JP
...
msg_print("モンスターを召喚した！");
		...
		for (k = 0;k < plev / 15 + 2; k++)
			if(summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, 0, (p_mode | u_mode)))
			{
				if (!pet)
#ifdef JP
msg_print("召喚されたモンスターは怒っている！");
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_ANT, (PM_ALLOW_GROUP | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_SPIDER, (PM_ALLOW_GROUP | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_HOUND, (PM_ALLOW_GROUP | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_HYDRA, (g_mode | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_ANGEL, (g_mode | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_DEMON, (g_mode | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_UNDEAD, (g_mode | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_DRAGON, (g_mode | p_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_HI_UNDEAD, (g_mode | p_mode | u_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_HI_DRAGON, (g_mode | p_mode | u_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_AMBERITES, (g_mode | p_mode | u_mode)))
			{
				if (!pet)
#ifdef JP
...
#endif
		for (k = 0;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_UNIQUE, (g_mode | p_mode | PM_ALLOW_UNIQUE)))
			{
				count++;
				if (!pet)
#ifdef JP
...
msg_print("Summoned special opponents are angry!");
		...
		for (k = count;k < 1; k++)
			if (summon_specific((pet ? -1 : 0), py, p_ptr->fx, summon_lev, SUMMON_HI_UNDEAD, (g_mode | p_mode | PM_ALLOW_UNIQUE)))
			{
				count++;
				if (!pet)
#ifdef JP
