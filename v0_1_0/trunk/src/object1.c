...
info[i++] = "それは神に祝福されている。";
	...
	if (object_is_cursed(o_ptr))
	{
		if (o_ptr->curse_flags & TRC_DIVINE_CURSE)
		{
			...
			tx[0] = '\0';
#ifdef JP
			sprintf(tx, "それは神格 %d の神域の呪いがかけられている。", o_ptr->xtra1);
#else
...
#ifdef JP
		case INVEN_3RDARM:
			p = cr_ptr->heavy_wield[1] ? "運搬中" : "第３手"; break;
#else
		case INVEN_3RDARM:
...
#ifdef JP
		case INVEN_4THARM:
			p = cr_ptr->heavy_wield[1] ? "運搬中" : "第４手"; break;
#else
		case INVEN_4THARM:
			p = cr_ptr->heavy_wield[1] ? "Just lifting" : "On arm"; break;
#endif
...
#ifdef JP
		case INVEN_5THARM:
			p = cr_ptr->heavy_wield[1] ? "運搬中" : "第５手"; break;
#else
		case INVEN_5THARM:
			p = cr_ptr->heavy_wield[1] ? "Just lifting" : "On arm"; break;
#endif
...
#ifdef JP
		case INVEN_6THARM:
			p = cr_ptr->heavy_wield[1] ? "運搬中" : "第６手"; break;
#else
		case INVEN_6THARM:
			p = cr_ptr->heavy_wield[1] ? "Just lifting" : "On arm"; break;
#endif
...
#ifdef JP
		case INVEN_1STHEAD:
			p = cr_ptr->num_head > 1 ? "頭部１" : " 頭部"; break;
#else
		case INVEN_1STHEAD:
			p = "On head"; break;
#endif
...
#ifdef JP
		case INVEN_1STHANDS:
			p = cr_ptr->num_hand > 2 ? "腕部１" : " 腕部"; break;
#else
		case INVEN_1STHANDS:
			p = "On hands"; break;
#endif
...
int show_inven(int target_item, creature_type *cr_ptr, bool right_set)
{
	...
	{
#if JP
		put_str("[何も持っていない]", 1, right_set ? wid - 20 : 1);
#else
...
int show_equip(int target_item, creature_type *cr_ptr, bool right_set)
{
	...
	{
#if JP
		put_str("[何も装備していない]", 1, right_set ? wid - 22 : 1);
#else
...
static bool get_item_okay(creature_type *cr_ptr, int i)
{
	...
	/* Verify the item */
	if (!item_tester_okay(cr_ptr, &cr_ptr->inventory[i], NULL)) return (FALSE);
...
bool get_item(creature_type *cr_ptr, int *cp, cptr pmt, cptr str, int mode, bool (*hook)(creature_type *cr_ptr, object_type *o_ptr))
{
	...
	if (repeat_pull(cp))
	{
		...
		else if (floor && (*cp < 0))
		{
			...
			/* Validate the item */
			if (item_tester_okay(p_ptr, o_ptr, hook))
			{
				/* Forget restrictions */
				item_tester_tval = 0;
				hook = NULL;
	...
	else if (use_menu)
	{
		for (j = 0; j < INVEN_PACK; j++)
			if (item_tester_okay(p_ptr, &cr_ptr->inventory[j], hook)) max_inven++;
	}
	...
	else if (use_menu)
	{
		for (j = INVEN_1STARM; j < INVEN_TOTAL; j++)
			if (select_ring_slot ? is_ring_slot(j) : item_tester_okay(p_ptr, &cr_ptr->inventory[j], hook)) max_equip++;
	...
	if (floor)
	{
		...
		for (this_o_idx = cave[cr_ptr->fy][cr_ptr->fx].o_idx; this_o_idx; this_o_idx = next_o_idx)
		{
			...
			if (item_tester_okay(p_ptr, o_ptr, hook) && (o_ptr->marked & OM_FOUND)) allow_floor = TRUE;
		}
	}
			...
			case '-':
			{
				...
				if (allow_floor)
				{
					...
					for (this_o_idx = cave[cr_ptr->fy][cr_ptr->fx].o_idx; this_o_idx; this_o_idx = next_o_idx)
					{
						...
						/* Validate the item */
						if (!item_tester_okay(p_ptr, o_ptr, hook)) continue;
...
bool get_item_floor(creature_type *cr_ptr, int *cp, cptr pmt, cptr str, int mode)
{
	...
	if (repeat_pull(cp))
	{
		...
		else if (floor && (*cp < 0))
		{
			...
			/* Validate the item */
			else if (item_tester_okay(cr_ptr, &o_list[0 - (*cp)], item_tester_hook))
			{
				/* Forget restrictions */
				item_tester_tval = 0;
				item_tester_hook = NULL;
	...
	{
		for (j = 0; j < INVEN_PACK; j++)
			if (item_tester_okay(cr_ptr, &cr_ptr->inventory[j], item_tester_hook)) max_inven++;
	}
	...
	{
		for (j = INVEN_1STARM; j < INVEN_TOTAL; j++)
			if (select_ring_slot ? is_ring_slot(j) : item_tester_okay(cr_ptr, &cr_ptr->inventory[j], item_tester_hook)) max_equip++;
		if (cr_ptr->ryoute && !item_tester_no_ryoute) max_equip++;
	}
