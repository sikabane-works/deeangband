...
static bool item_monster_okay(int species_idx)
{
	...
	if (is_resist_all_species(r_ptr)) return (FALSE);
...
void apply_magic(object_type *o_ptr, int lev, u32b mode)
{
	...
	if (have_creature_flags(p_ptr, CF_GOOD_LUCK))
	{
		f1 += 5;
		f2 += 2;
	}
	else if(have_creature_flags(p_ptr, CF_BAD_LUCK))
	{
		f1 -= 5;
		f2 -= 2;
	}
	...
	for (i = 0; i < rolls; i++)
	{
		...
		if (have_creature_flags(p_ptr, CF_GOOD_LUCK) && one_in_(77))
		{
			if (make_artifact(o_ptr)) break;
		}
	}
...
static void spell_damcalc(creature_type *m_ptr, int typ, int dam, int limit, int *max)
{
	...
	case GF_ELEC:
		...
		{
			if (have_creature_flags(p_ptr, CF_VULN_ELEM)) dam *= 2;
