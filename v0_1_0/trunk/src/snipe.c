...
int tot_dam_aux_snipe(creature_type *cr_ptr, int mult, creature_type *m_ptr)
{
	...
	case SP_LITE:
		...
		break;
	case SP_FIRE:
		if (r_ptr->flags10 & RFR_IM_FIRE)
		{
			if (seen) r_ptr->r_flags10 |= RFR_IM_FIRE;
		}
		else
		{
	...
	case SP_COLD:
		...
		{
			if (seen) r_ptr->r_flags10 |= RFR_IM_COLD;
		}
		else
		{
			int n = 15 + (cr_ptr->concent * 3);
		...
		break;
	case SP_ELEC:
		if (r_ptr->flags10 & RFR_IM_ELEC)
		{
			if (seen) r_ptr->r_flags10 |= RFR_IM_ELEC;
		}
		else
		{
