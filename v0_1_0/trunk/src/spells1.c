...
note_dies = "は蒸発した！";
		...
		case GF_PSI:
		{
			...
			if (!(los(m_ptr->fy, m_ptr->fx, py, p_ptr->fx)))
			{
#ifdef JP
...
note_dies = "はドロドロに溶けた！";
		...
		case GF_CAUSE_4:
		{
			...
			/* Attempt a saving throw */
			if ((randint0(100 + (caster_lev / 2)) < (r_ptr->level + 35)) && ((who_ptr == who_ptr) || (who_ptr->monster_idx != MON_KENSHIROU)))
			{
#ifdef JP
				note = "には効果がなかった。";
#else
...
note = "には効果がなかった。";
	...
	else if (who_ptr != who_ptr)
	{
		...
		if (m_ptr->chp < 0)
		{
			...
			//TODO
			//if (who_ptr != who_ptr) monster_gain_exp(who_ptr, who, m_ptr->monster_idx);
...
msg_print("エネルギーのうねりを感じた！");
			...
			case 19: case 20: case 21: case 22:
			{
				...
				count += summon_specific((pet ? -1 : 0), py, p_ptr->fx, (pet ? who_ptr->lev*2/3+randint1(who_ptr->lev/2) : dun_level), 0, mode);
				if (!one_in_(6)) break;
			}
...
static bool project_p(creature_type *who_ptr, cptr who_name, int r, int y, int x, int dam, int typ, int flg, int monspell)
{
	...
	if ((p_ptr->reflect || ((p_ptr->special_defense & KATA_FUUJIN) && !p_ptr->blind)) && (flg & PROJECT_REFLECTABLE) && !one_in_(10))
	{
		...
		if (!is_player(who_ptr))
		{
			...
			while (max_attempts && in_bounds2u(t_y, t_x) && !projectable(py, p_ptr->fx, t_y, t_x));
...
*/
	...
	switch (typ)
	{
		...
		case GF_CAUSE_4:
		{
			if ((randint0(100 + rlev / 2) < p_ptr->skill_rob) && !(who_ptr->monster_idx == MON_KENSHIROU) && !(p_ptr->multishadow && (turn & 1)))
			{
#ifdef JP
...
bool project(creature_type *who_ptr, int rad, int y, int x, int dam, int typ, int flg, int monspell)
{
	...
	if (flg & PROJECT_KILL)
	{
		see_s_msg = (!is_player(who_ptr)) ? is_seen(p_ptr, who_ptr) :
			(is_player(who_ptr) ? TRUE : (player_can_see_bold(y1, x1) && projectable(py, p_ptr->fx, y1, x1)));
	}
...
bool binding_field( int dam )
{
	...
	for( x=0 ; x < cur_wid ; x++ )
	{
		...
		{
			if( is_mirror_grid(&cave[y][x]) &&
			    distance(py,p_ptr->fx,y,x) <= MAX_RANGE(p_ptr) &&
			    distance(py,p_ptr->fx,y,x) != 0 &&
	...
	for( y=y1 ; y <=y2 ; y++ ){
		for( x=x1 ; x <=x2 ; x++ ){
			...
			{
				if (player_has_los_bold(y, x) && projectable(py, p_ptr->fx, y, x)) {
					/* Visual effects */
					if(!(p_ptr->blind)
					   && panel_contains(y,x)){
