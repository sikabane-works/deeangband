		...
		case GF_PSI:
		{
			         ...
			         (r_ptr->level > randint1(3 * dam)))
			{
				    ...
				    one_in_(2))
				{
					...
					/* Saving throw */
					if ((randint0(100 + r_ptr->level / 2) < p_ptr->skill_rob) && !(p_ptr->multishadow && (turn & 1)))
					{
#ifdef JP
					...
					else
					{
						...
						if (one_in_(4) && !(p_ptr->multishadow && (turn & 1)))
						{
							switch (randint1(4))
							{
								case 1:
...
note = "は動けなくなった！";
		...
		case GF_CHARM:
		{
			...
			if ((m_ptr->resist_ultimate) || p_ptr->inside_arena)
			{
#ifdef JP
				note = "には効果がなかった！";
#else
