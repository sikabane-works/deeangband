...
void self_knowledge(creature_type *cr_ptr)
{
	...
	cr_ptr->knowledge |= (KNOW_STAT | KNOW_HPRATE);
...
sprintf(Dummy, "現在の体力ランク : %d/100", percent);
	...
	for (v_nr = 0; v_nr < 6; v_nr++)
	{
		...
		sprintf(stat_desc, "%s 18/%d", stat_names[v_nr], cr_ptr->stat_max_max[v_nr]-18);
...
info[i++] = "あなたはランダムな方向に対して数回攻撃することができる。(75 MP)";
		...
		case CLASS_HIGH_MAGE:
			if (cr_ptr->realm1 == REALM_HEX) break;
		case CLASS_MAGE:
		case CLASS_SORCERER:
			if (plev > 24)
			{
...
info[i++] = "あなたは狂戦士化の発作を起こす。";
		...
		if (cr_ptr->trait2 & MUT2_COWARDICE)
		{
#ifdef JP
info[i++] = "あなたは時々臆病になる。";
#else
		...
		if (cr_ptr->trait2 & MUT2_RTELEPORT)
		{
#ifdef JP
...
info[i++] = "あなたは幻覚を引き起こす精神錯乱に侵されている。";
		...
		if (cr_ptr->trait2 & MUT2_FLATULENT)
		{
#ifdef JP
info[i++] = "あなたは制御できない強烈な屁をこく。";
		...
		if (cr_ptr->trait2 & MUT2_PROD_MANA)
		{
#ifdef JP
info[i++] = "あなたは制御不能な魔法のエネルギーを発している。";
		...
		if (cr_ptr->trait2 & MUT2_ATT_DEMON)
		{
#ifdef JP
info[i++] = "あなたはデーモンを引きつける。";
		...
		if (cr_ptr->trait2 & MUT2_SCOR_TAIL)
		{
#ifdef JP
...
info[i++] = "あなたはクチバシが生えている。(ダメージ 2d4)";
		...
		if (cr_ptr->trait2 & MUT2_SPEED_FLUX)
		{
#ifdef JP
info[i++] = "あなたはランダムに早く動いたり遅く動いたりする。";
		...
		if (cr_ptr->trait2 & MUT2_BANISH_ALL)
		{
#ifdef JP
info[i++] = "あなたは時々近くのモンスターを消滅させる。";
		...
		if (cr_ptr->trait2 & MUT2_EAT_LIGHT)
		{
#ifdef JP
...
info[i++] = "あなたは象のような鼻を持っている。(ダメージ 1d4)";
		...
		if (cr_ptr->trait2 & MUT2_ATT_ANIMAL)
		{
#ifdef JP
info[i++] = "あなたは動物を引きつける。";
#else
		...
		if (cr_ptr->trait2 & MUT2_TENTACLES)
		{
#ifdef JP
info[i++] = "あなたは邪悪な触手を持っている。(ダメージ 2d5)";
		...
		if (cr_ptr->trait2 & MUT2_RAW_CHAOS)
		{
#ifdef JP
info[i++] = "あなたはしばしば純カオスに包まれる。";
		...
		if (cr_ptr->trait2 & MUT2_NORMALITY)
		{
#ifdef JP
...
info[i++] = "あなたの肉体は幽体化したり実体化したりする。";
		...
		if (cr_ptr->trait2 & MUT2_POLY_WOUND)
		{
#ifdef JP
...
info[i++] = "あなたは衰弱する恐ろしい病気にかかっている。";
		...
		if (cr_ptr->trait2 & MUT2_ATT_DRAGON)
		{
#ifdef JP
info[i++] = "あなたはドラゴンを引きつける。";
		...
		if (cr_ptr->trait2 & MUT2_WEIRD_MIND)
		{
#ifdef JP
...
info[i++] = "あなたの胃は非常に落ち着きがない。";
		...
		if (cr_ptr->trait2 & MUT2_CHAOS_GIFT)
		{
#ifdef JP
info[i++] = "あなたはカオスの守護悪魔から褒美をうけとる。";
		...
		if (cr_ptr->trait2 & MUT2_WALK_SHAD)
		{
#ifdef JP
...
info[i++] = "あなたは時々負け知らずな気分になる。";
		...
		if (cr_ptr->trait2 & MUT2_SP_TO_HP)
		{
#ifdef JP
info[i++] = "あなたは時々血が筋肉にどっと流れる。";
		...
		if (cr_ptr->trait2 & MUT2_HP_TO_SP)
		{
#ifdef JP
...
info[i++] = "あなたはよくつまづいて物を落とす。";
	...
	if (cr_ptr->trait3)
	{
		if (cr_ptr->trait3 & MUT3_HYPER_STR)
		{
#ifdef JP
...
info[i++] = "あなたは精神薄弱だ。(知能＆賢さ-4)";
		...
		if (cr_ptr->trait3 & MUT3_RESILIENT)
		{
#ifdef JP
info[i++] = "あなたは非常にタフだ。(耐久+4)";
		...
		if (cr_ptr->trait3 & MUT3_XTRA_FAT)
		{
#ifdef JP
...
info[i++] = "あなたはアルビノだ。(耐久-4)";
		...
		if (cr_ptr->trait3 & MUT3_FLESH_ROT)
		{
#ifdef JP
info[i++] = "あなたの肉体は腐敗している。(耐久-2,魅力-1)";
		...
		if (cr_ptr->trait3 & MUT3_SILLY_VOI)
		{
#ifdef JP
info[i++] = "あなたの声は間抜けなキーキー声だ。(魅力-4)";
		...
		if (cr_ptr->trait3 & MUT3_BLANK_FAC)
		{
#ifdef JP
info[i++] = "あなたはのっぺらぼうだ。(魅力-1)";
		...
		if (cr_ptr->trait3 & MUT3_ILL_NORM)
		{
#ifdef JP
info[i++] = "あなたは幻影に覆われている。";
		...
		if (cr_ptr->trait3 & MUT3_XTRA_EYES)
		{
#ifdef JP
info[i++] = "あなたは余分に二つの目を持っている。(探索+15)";
		...
		if (cr_ptr->trait3 & MUT3_MAGIC_RES)
		{
#ifdef JP
info[i++] = "あなたは魔法への耐性をもっている。";
		...
		if (cr_ptr->trait3 & MUT3_XTRA_NOIS)
		{
#ifdef JP
info[i++] = "あなたは変な音を発している。(隠密-3)";
		...
		if (cr_ptr->trait3 & MUT3_INFRAVIS)
		{
#ifdef JP
info[i++] = "あなたは素晴らしい赤外線視力を持っている。(+3)";
		...
		if (cr_ptr->trait3 & MUT3_XTRA_LEGS)
		{
#ifdef JP
info[i++] = "あなたは余分に二本の足が生えている。(加速+3)";
		...
		if (cr_ptr->trait3 & MUT3_SHORT_LEG)
		{
#ifdef JP
info[i++] = "あなたの足は短い突起だ。(加速-3)";
		...
		if (cr_ptr->trait3 & MUT3_ELEC_TOUC)
		{
#ifdef JP
info[i++] = "あなたの血管には電流が流れている。";
		...
		if (cr_ptr->trait3 & MUT3_FIRE_BODY)
		{
#ifdef JP
info[i++] = "あなたの体は炎につつまれている。";
		...
		if (cr_ptr->trait3 & MUT3_WART_SKIN)
		{
#ifdef JP
...
info[i++] = "あなたの肌は鱗になっている。(魅力-1, AC+10)";
		...
		if (cr_ptr->trait3 & MUT3_IRON_SKIN)
		{
#ifdef JP
...
info[i++] = "あなたは羽を持っている。";
		...
		if (cr_ptr->trait3 & MUT3_REGEN)
		{
			/* Unnecessary */
		}
		if (cr_ptr->trait3 & MUT3_ESP)
		{
			/* Unnecessary */
		}
		if (cr_ptr->trait3 & MUT3_LIMBER)
		{
#ifdef JP
info[i++] = "あなたの体は非常にしなやかだ。(器用+3)";
		...
		if (cr_ptr->trait3 & MUT3_ARTHRITIS)
		{
#ifdef JP
info[i++] = "あなたはいつも関節に痛みを感じている。(器用-3)";
		...
		if (cr_ptr->trait3 & MUT3_VULN_ELEM)
		{
#ifdef JP
info[i++] = "あなたは元素の攻撃に弱い。";
#else
...
info[i++] = "あなたは白いオーラにつつまれている。";
		...
		if (cr_ptr->trait3 & MUT3_BAD_LUCK)
		{
#ifdef JP
...
info[i++] = "あなたは邪悪な怨念に包まれている。";
	...
	if (cr_ptr->cursed & TRC_AGGRAVATE)
	{
#ifdef JP
info[i++] = "あなたはモンスターを怒らせている。";
	...
	if (cr_ptr->cursed & TRC_DRAIN_EXP)
	{
#ifdef JP
...
info[i++] = "あなたの回復力は非常に遅い。";
	...
	if (cr_ptr->cursed & TRC_ADD_L_CURSE)
	{
#ifdef JP
info[i++] = "あなたの弱い呪いは増える。"; /* 暫定的 -- henkma */
	...
	if (cr_ptr->cursed & TRC_ADD_H_CURSE)
	{
#ifdef JP
info[i++] = "あなたの強い呪いは増える。"; /* 暫定的 -- henkma */
	...
	if (cr_ptr->cursed & TRC_CALL_ANIMAL)
	{
#ifdef JP
info[i++] = "あなたは動物に狙われている。";
	...
	if (cr_ptr->cursed & TRC_CALL_DEMON)
	{
#ifdef JP
info[i++] = "あなたは悪魔に狙われている。";
	...
	if (cr_ptr->cursed & TRC_CALL_DRAGON)
	{
#ifdef JP
info[i++] = "あなたはドラゴンに狙われている。";
	...
	if (cr_ptr->cursed & TRC_COWARDICE)
	{
#ifdef JP
info[i++] = "あなたは時々臆病になる。";
#else
...
info[i++] = "あなたの位置はひじょうに不安定だ。";
	...
	if (cr_ptr->cursed & TRC_LOW_MELEE)
	{
#ifdef JP
...
info[i++] = "あなたは攻撃を受けやすい。";
	...
	if (cr_ptr->cursed & TRC_LOW_MAGIC)
	{
#ifdef JP
...
info[i++] = "あなたは体力を吸われている。";
	...
	if (cr_ptr->cursed & TRC_DRAIN_MANA)
	{
#ifdef JP
...
info[i++] = "あなたは酸に対する完全なる免疫を持っている。";
	...
	else if (cr_ptr->resist_acid && IS_OPPOSE_ACID(cr_ptr))
	{
#ifdef JP
...
info[i++] = "あなたは電撃に対する完全なる免疫を持っている。";
	...
	else if (cr_ptr->resist_elec && IS_OPPOSE_ELEC(cr_ptr))
	{
#ifdef JP
...
info[i++] = "あなたは電撃への耐性を持っている。";
	...
	if (race_is_(cr_ptr, RACE_ANDROID) && !cr_ptr->immune_elec)
	{
#ifdef JP
info[i++] = "あなたは電撃に弱い。";
#else
...
info[i++] = "あなたは火に対する完全なる免疫を持っている。";
	...
	else if (cr_ptr->resist_fire && IS_OPPOSE_FIRE(cr_ptr))
	{
#ifdef JP
...
info[i++] = "あなたは火への耐性を持っている。";
	...
	if (race_is_(cr_ptr, RACE_ENT) && !cr_ptr->immune_fire)
	{
#ifdef JP
info[i++] = "あなたは火に弱い。";
#else
...
info[i++] = "あなたは冷気に対する完全なる免疫を持っている。";
	...
	else if (cr_ptr->resist_cold && IS_OPPOSE_COLD(cr_ptr))
	{
#ifdef JP
...
info[i++] = "あなたは毒への強力な耐性を持っている。";
	...
	else if (cr_ptr->resist_pois || IS_OPPOSE_POIS(cr_ptr))
	{
#ifdef JP
...
bool genocide_aux(int m_idx, int power, bool player_cast, int dam_side, cptr spell_name)
{
	...
	if (player_cast)
	{
		/* Take damage */
#ifdef JP
		take_hit(NULL, p_ptr, DAMAGE_GENO, randint1(dam_side), format("%^sの呪文を唱えた疲労", spell_name), NULL, -1);
#else
