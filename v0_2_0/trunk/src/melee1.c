...
bool special_melee(creature_type *atk_ptr, creature_type *tar_ptr)
{
	...
	{
#ifdef JP
		msg_format("%sÇÕëäéËÇ™èPÇ¢Ç©Ç©ÇÈëOÇ…ëfëÅÇ≠ïêäÌÇêUÇÈÇ¡ÇΩÅB", taspecies_name);
#else
...
#ifdef JP
				if (abbreviate == 0)
				    msg_format("%^sÇÕ%sÇ%s", atk_name, taspecies_name, act);
				else if (abbreviate == 1)
				    msg_format("%^sÇÕ%sÇ…%s", atk_name, taspecies_name, act);
				else if (abbreviate == 2)
				    msg_format("%^sÇÕ%sÇÃ%s", atk_name, taspecies_name, act);
				else if (abbreviate == 3)
				    msg_format("%^sÇÕ%sÇ…å¸ÇØ%s", atk_name, taspecies_name, act);
