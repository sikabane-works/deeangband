...
#ifdef JP
	case 10: sprintf(p, " 最大重量:%dkg", plev * 15);  break;
#else
...
#ifdef JP
	case 13: sprintf(p, " 行動:%ld回", (cr_ptr->csp + 100-cr_ptr->energy_need - 50)/100); break;
#else
...
msg_print("その方向にはモンスターはいません。");
	...
	case 12:
	{
		...
		path_n = project_path(path_g, MAX_RANGE(cr_ptr), target_row, target_col, cr_ptr->fy, cr_ptr->fx, 0);
