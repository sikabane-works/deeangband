...
static void learned_info(creature_type *cr_ptr, char *p, int power)
{
	...
	switch (power)
	{
		...
		case MS_SHOOT:
		{
			...
			if (get_equipped_slot_num(cr_ptr, INVEN_SLOT_HAND) > 0) o_ptr = &cr_ptr->inventory[INVEN_1STARM];
			else if (get_equipped_slot_num(cr_ptr, INVEN_SLOT_HAND) > 1) o_ptr = &cr_ptr->inventory[INVEN_2NDARM];
			else
				sprintf(p, " %s1", s_dam);
			if (o_ptr)
...
#endif
			if (get_equipped_slot_num(caster_ptr, INVEN_SLOT_HAND) > 0) o_ptr = &caster_ptr->inventory[INVEN_1STARM];
			else if (get_equipped_slot_num(caster_ptr, INVEN_SLOT_HAND) > 1) o_ptr = &caster_ptr->inventory[INVEN_2NDARM];
			else
			damage = 1;
			if (o_ptr)
			{
...
msg_format("%sを引き戻した。", m_name);
		...
		teleport_creature_to2(cave[target_row][target_col].m_idx, caster_ptr, caster_ptr->fy, caster_ptr->fx, 100, TELEPORT_PASSIVE);
		break;
	}
	case MS_TELE_AWAY:
		if (!get_aim_dir(caster_ptr, &dir)) return FALSE;
