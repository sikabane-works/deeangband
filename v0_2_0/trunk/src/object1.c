...
cptr describe_use(creature_type *cr_ptr, int i)
{
	...
	{
#ifdef JP
		case INVEN_SLOT_HAND:  p = cr_ptr->heavy_wield[0] ? "運搬中の" : ((cr_ptr->two_handed && cr_ptr->migite) ? "両手に装備している" : (has_cf_creature(cr_ptr, CF_LEFT_HANDER) ? "左手に装備している" : "右手に装備している")); break;
#else
		case INVEN_SLOT_HAND:  p = cr_ptr->heavy_wield[0] ? "just lifting" : (cr_ptr->migite ? "attacking monsters with" : "wearing on your arm"); break;
			...
			/*
#ifdef JP
		case INVEN_SLOT_HAND:  p = cr_ptr->heavy_wield[1] ? "運搬中の" : ((cr_ptr->two_handed && cr_ptr->hidarite) ? "両手に装備している" : (has_cf_creature(cr_ptr, CF_LEFT_HANDER) ? "右手に装備している" : "左手に装備している")); break;
#else
...
int show_item_slot(int target_item, creature_type *cr_ptr, u32b flags, bool (*hook)(creature_type *cr_ptr, object_type *o_ptr))
{
	...
	if(!k)
	{
		...
		{
#if JP
			put_str("[何も装備していない]", 1, flags & SHOW_ITEM_RIGHT_SET ? wid - 23 : 1);
#else
 ...
 */
bool get_item_floor(creature_type *cr_ptr, int *cp, cptr pmt, cptr str, int mode, bool (*hook)(creature_type *cr_ptr, object_type *o_ptr))
{
	...
	while (!done)
	{
		...
		switch (which)
		{
			...
			default:
			{
				...
				/* Validate the item */
				if ((command_wrk != USE_FLOOR) && !get_item_okay(cr_ptr, k, hook))
				{
					bell();
					break;
				}
