...
static bool project_m(creature_type *caster_ptr, int r, int y, int x, int dam, int typ, int flg, bool see_s_msg)
{
	...
	bool known = ((target_ptr->cdis <= MAX_SIGHT) || inside_battle);
...
//
	...
	if ((is_quest_creature(target_ptr)) || is_unique_species(species_ptr) || has_cf_creature(target_ptr, CF_NAZGUL) && !inside_battle)
	{
...
note = "には耐性がある！";
		...
		case GF_CAPTURE:
		{
			...
			{
				if (target_ptr->mflag2 & MFLAG2_CHAMELEON) choose_new_monster(c_ptr->m_idx, FALSE, MON_CHAMELEON, MONEGO_NONE);
#ifdef JP
...
static bool project_creature(creature_type *atk_ptr, cptr who_name, int r, int y, int x, int dam, int typ, int flg, bool see_s_msg, int spell)
{
	...
	if ((player_ptr->reflect || ((player_ptr->special_defense & KATA_FUUJIN) && !player_ptr->blind)) && (flg & PROJECT_REFLECTABLE) && !one_in_(10))
	{
		...
		if (!is_player(atk_ptr))
		{
			...
			while (max_attempts && in_bounds2u(t_y, t_x) && !projectable(p_ptr, player_ptr->fy, player_ptr->fx, t_y, t_x));
			...
			for (y = by - cdis; y <= by + cdis; y++)
			{
				for (x = bx - cdis; x <= bx + cdis; x++)
				{
					...
					default:
						...
						if (!projectable(p_ptr, by, bx, y, x)) continue;
						break;
					}
	...
	if (flg & PROJECT_KILL)
	{
		...
		if(caster_ptr)
		{
			see_s_msg = (!is_player(caster_ptr)) ? is_seen(caster_ptr, caster_ptr) :
				(is_player(caster_ptr) ? TRUE : (player_can_see_bold(caster_ptr, y1, x1) && projectable(p_ptr, caster_ptr->fy, caster_ptr->fx, y1, x1)));
		}
		else
		{
			see_s_msg = TRUE;
		...
		for (i = 0; i < grids; i++)
		{
			...
			if (grids <= 1)
			{
				    ...
				    ((caster_ptr && is_player(caster_ptr)) || dist_hack > 1) && !one_in_(10))
				{
					...
					while (max_attempts && in_bounds2u(t_y, t_x) && !projectable(p_ptr, y, x, t_y, t_x));
...
bool binding_field(creature_type *caster_ptr, int dam)
{
	...
	for( x=0 ; x < cur_wid ; x++ )
	{
		for( y=0 ; y < cur_hgt ; y++ )
		{
			if( is_mirror_grid(&cave[y][x]) &&
			    ...
			    projectable(p_ptr, caster_ptr->fy, caster_ptr->fx, y, x)
			    ){
				mirror_y[mirror_num]=y;
				mirror_x[mirror_num]=x;
				mirror_num++;
	...
	for( y=y1 ; y <=y2 ; y++ ){
		for( x=x1 ; x <=x2 ; x++ ){
			...
			{
				if (player_has_los_bold(y, x) && projectable(p_ptr, caster_ptr->fy, caster_ptr->fx, y, x)) {
					/* Visual effects */
					if(!(caster_ptr->blind)
					   && panel_contains(y,x)){
