...
bool probing(void)
{
	...
	for (i = 1; i < m_max; i++)
	{
		...
		if (m_ptr->ml)
		{
			...
			creature_desc(m_name, cr_ptr, m_ptr, MD_IGNORE_HALLU | MD_INDEF_HIDDEN);
	...
	if (hurt && !target_ptr->pass_wall && !target_ptr->kill_wall)
	{
		...
		if (damage)
		{
			...
			if (m_idx)
			{
				...
				creature_desc(m_name, cr_ptr, m_ptr, MD_IGNORE_HALLU | MD_ASSUME_VISIBLE | MD_INDEF_VISIBLE);
...
void discharge_minion(creature_type *caster_ptr)
{
	...
	for (i = 1; i < m_max; i++)
	{
		...
		{
			char m_name[80];
			creature_desc(m_name, cr_ptr, m_ptr, 0x00);
#ifdef JP
		...
		if (c_ptr->m_idx)
		{
			...
			if (m_ptr->paralyzed && (randint0(100) < chance))
			{
				...
				if (m_ptr->ml)
				{
					...
					creature_desc(m_name, p_ptr, m_ptr, 0);
...
bool kawarimi(creature_type *user_ptr, bool success)
{
	...
	int y, x;
	char user_name[80];
	creature_desc(user_name, user_ptr, user_ptr, 0);
